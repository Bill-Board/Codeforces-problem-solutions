...
void solve()
{
    ...
    {
        if(s[i]==s[j] and s[i]=='a')continue;
        print("YES");
        if(s[i]==s[j])
        {
            for(v99 k=0;k<i;k++)cout<<s[k];cout<<"a";
            for(v99 k=i;k<n;k++)cout<<s[k];cout<<ses;
            return;
        }
        else
        {
            if(s[j]=='a')
            {
                for(v99 k=0;k<j;k++)cout<<s[k];cout<<"a";
                for(v99 k=j;k<n;k++)cout<<s[k];cout<<ses;
            }
            else
            {
                for(v99 k=0;k<i;k++)cout<<s[k];cout<<"a";
