...
void solve()
{
    ...
    map<v99,v99>mpp;
    multiset<v99>s;
    rep0(i,0,n){v99 x;read(x);mpp[x]++;}
    for(auto i:mpp)s.insert(i.ss);
    while(s.size()>1)
    {
        v99 choto=*s.begin(),boro=*prev(s.end());
        s.erase(s.find(choto));s.erase(s.find(boro));
        if(choto!=1)s.insert(choto-1LL);
        if(boro!=1)s.insert(boro-1LL);
    }
    v99 cr_h=0;
    for(v99 i:s)cr_h+=i;
    print(cr_h);
