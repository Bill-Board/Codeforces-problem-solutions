...
map<v99,v99>ache;
v99 pre[mxi];
void segtree(v99 start,v99 enn)
{
    if(start>=enn)return;
    if(ok.find({start,enn})!=ok.end())return;
    ok[{start,enn}]++;
    ache[pre[enn]-pre[start]]++;
    v99 minimum=a[start],maximum=a[enn-1];
    v99 mid=(minimum+maximum)/2;
    v99 pos=(upper_bound(sob(a),mid)-a.begin());
    segtree(start,pos);
    segtree(pos,enn);
}
...
{
    /// code is here->
    v99 q,n;read(n,q);a.resize(n);
    read(a);sort(sob(a));pre[0]=0;
    rep0(i,0,n)pre[i+1]=pre[i]+a[i];
    segtree(0LL,n);
    ///print(ache);
    while(q--)
    {
        v99 sum;cin>>sum;
        if(ache.find(sum)!=ache.end())print("YES");
        else print("NO");
    }ache.clear();ok.clear();
}
