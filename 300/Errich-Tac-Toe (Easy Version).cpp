...
{
    /// code is here->
    v99 n;read(n);vector<string>v(n);
    v99 cnt[4]={};
    rep0(i,0,n)
    {
        cin>>v[i];
        rep0(j,0,n)if(v[i][j]=='X')cnt[(i+j)%3]++;
    }
    v99 pos=0,val=LLONG_MAX;
    rep0(i,0,3)if(val>cnt[i])val=cnt[i],pos=i;
    rep0(i,0,n)
    {
        rep0(j,0,n)if(v[i][j]=='X' and (i+j)%3==pos)v[i][j]='O';
    }
    rep0(i,0,n)
    {
