...
v99 n,k,A,B;
vector<v99>v;
///v99 pos=upper_bound(sob(v),y)-upper_bound(sob(v),x-1);
v99 dfs(v99 l, v99 r)
{
    v99 pos=upper_bound(sob(v),r)-lower_bound(sob(v),l);
    if(l==r)
    {
        if(pos>0)return B*pos;
        else return A;
    }
    v99 mid=(l+r)/2;
    if(pos>0)return min((r-l+1)*pos*B,dfs(l,mid)+dfs(mid+1,r));
    else return A;
}
