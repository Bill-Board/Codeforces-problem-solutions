...
v99 tr[15*mxi],a[2*mxi];
vll tom[2*mxi];

v99 koita_ache(v99 l, v99 r,v99 c)
{
    v99 kot=upper_bound(sob(tom[c]),r)-lower_bound(sob(tom[c]),l);
    return kot;
}

void build_seg(v99 node,v99 start,v99 endd)
{
    if(start==endd)
    {
        tr[node]=a[start];
    ...
    v99 mid=(start+endd)/2;
    build_seg(2*node,start,mid);
    build_seg(2*node+1,mid+1,endd);
    tr[node]=koita_ache(start,endd,tr[2*node])>koita_ache(start,endd,tr[2*node +1])?tr[2*node]:tr[2*node +1];
}
v99 query(v99 node,v99 start,v99 endd,v99 l,v99 r)
{
    if(r<start or endd<l or r<l)return 0LL;
    if(l<=start and endd<=r)return koita_ache(l,r,tr[node]);
    v99 mid=(start+endd)/2;
    v99 ek=query(2*node,start,mid,l,r),dui=query(2*node +1,mid+1,endd,l,r);
    return max(ek,dui);
}
...
void solve()
{
    ...
    {
        v99 l,r;read(l,r);
        v99 kot=query(1LL,1LL,n,l,r);
        print(max(1LL,2*kot-(r-l+1)));
    }
}
