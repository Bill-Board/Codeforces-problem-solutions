...
{
    /// code is here->
    v99 n,os=0,es=0;read(n);vll a(n);
    priority_queue<v99>e,o;
    for(auto &i:a)
    {
        cin>>i;
        if(i&1)o.push(i);
    ...
    for(v99 i=0;i<n;i++)
    {
        ...
        if(i%2==0)///alice
        {
            ...
            {
                v99 ek=e.top(),dui=-1;
                if(!o.empty())dui=o.top();
                if(ek>=dui)es+=ek,e.pop();
                else o.pop();
            }
        }
        else
        {
             ...
             else
             {
                v99 ek=-1,dui=o.top();
                if(!e.empty())ek=e.top();
                if(ek<=dui)os+=dui,o.pop();
                else e.pop();
             }
        }
    }
    if(es>os)print("Alice");
    else if(es==os)print("Tie");
    else print("Bob");
