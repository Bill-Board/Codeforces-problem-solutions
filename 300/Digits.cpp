...
///-------------------------------------------------------------------------------------------------------
vector<v99>tom[11],tom2[12];
void init()
{
    for(v99 i=1;i<10;i++)
    {
        v99 kot=1,lopp=min(4LL,cnt[i]);//print(lopp);
        for(v99 j=0;j<=lopp ;j++) tom[i].pb(kot),kot=(kot*i)%10;
    }
    //for(v99 i=1;i<10;i++)print(tom[i]);
}
v99 ans[12],p[12],n,d,cr_h=0,lp[12];
bool bl=false;
void dfs(v99 koita,v99 sum)
{
    if(koita==10LL)
    {
        if(sum==d)
        {
            bool bll=false;
            //rep1(i,0,9)cout<<ans[i]<<whp;cout<<ses;
            v99 kot=1;
            for(v99 i=1;i<10;i++)
            {
                v99 ppp=(cnt[i]-ans[i])/4LL;ppp*=4LL;
                ppp+=(ans[i]);
                p[i]=ppp;
                kot*=tom2[i][ppp];
                if(ppp)bll=true;
            ...
            {
                cr_h=kot;
                for(v99 i=0;i<10;i++)lp[i]=p[i];//,cout<<lp[i]<<whp ses[i==9];
            }
        }
        return;
    }
    v99 kot=1;
    for(v99 i=0;i<tom[koita].size();i++)
    {
        v99 kot=sum*tom[koita][i];
        ans[koita]=i;
        dfs(koita+1,kot%10);
    }
}
...
void solve()
{
    ...
    read(n,d);
    v99 a[n+2];
    rep1(i,1,9)tom2[i].pb(1LL);
    rep1(i,1,n)read(a[i]),cnt[a[i]%10]++,tom2[a[i]%10].pb(a[i]);
    rep1(i,1,9)sort(tom2[i].begin()+1,tom2[i].end(),greater<>());
    rep0(i,0,10)cout<<cnt[i]<<whp ses[i==9];
    rep1(i,1,9)
    {
        if(tom2[i].size()>1)
        {
            rep0(j,1,tom[i].size())tom[i][j]*=tom[i][j-1];
        }
    }
    sort(a+1,a+n+1,greater<>());
    init();
    dfs(1LL,1LL);
    if(!bl)return print(-1);
    print(cr_h);
    vll r;
    for(v99 i=1;i<=n;i++)
    {
        if(a[i]%10==0 or a[i]%10==1)r.pb(a[i]);
        else if(lp[a[i]%10])r.pb(a[i]),lp[a[i]]--;
    }
    print(r.size());
    sort(sob(r));
    print(r);
