...
///-------------------------------------------------------------------------------------------------------
map<pii,v99>s;
v99 alabu(v99 l1,v99 r1,v99 c1,v99 l2,v99 r2,v99 c2)
{
    v99 ek=LLONG_MAX;
    if(s.find({l1,r2})!=s.end())ek=s[{l1,r2}];
    if(l1==l2)
    {
        if(r1==r2)return min({c1,c2,ek});
        else return min(ek,c2);
    }
    else return min(c1+c2,ek);
}
...
{
    /// code is here->
    v99 n;read(n);s.clear();
    v99 mnn=LLONG_MAX,mxx=-1,cr_h=LLONG_MAX;
    map<v99,set<pii> >mpp1,mpp2;
    rep0(i,0,n)
    {
        v99 l,r,c;read(l,r,c);
        if(s.find({l,r})==s.end())s[{l,r}]=c;
        else s[{l,r}]=min(c,s[{l,r}]);
        mnn=min(l,mnn);mxx=max(mxx,r);
        mpp1[l].insert({c,r});
        mpp2[r].insert({c,l});
        v99 ek=(*mpp1[mnn].begin()).ff,mnr=(*mpp1[mnn].begin()).ss;
        v99 dui=(*mpp2[mxx].begin()).ff,mxl=(*mpp2[mxx].begin()).ss;
        //print(mnn,mnr,ek,mxl,mxx,dui);
        print(alabu(mnn,mnr,ek,mxl,mxx,dui));
    }
}
