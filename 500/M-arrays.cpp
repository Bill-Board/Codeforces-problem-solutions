...
void solve()
{
    ...
    {
        auto it=mpp.begin();
        v99 ek=it->ff,koi=it->ss;
        v99 dui=ek==0?0:m-it->ff,koi2=mpp[dui];
        if(ek==dui)
        {
            cr_h++;
            mpp.erase(it);
        ...
        v99 kot=min(koi,koi2)+1;
        mpp[ek]-=kot;mpp[dui]-=kot;
        cr_h+=max(0LL,mpp[ek]);cr_h+=max(0LL,mpp[dui]);
        cr_h++;
        mpp.erase(it);mpp.erase(mpp.find(dui));
    }
    print(cr_h);
}
