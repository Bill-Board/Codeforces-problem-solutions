...
void solve()
{
    ...
    if(pach!=-1)
    {
        for(v99 i=pach-1;i>=0;i--)
        {
            if(n[i]-'0'== 2 or n[i]-'0'==7)
            {
                v99 len=n.size()-(i+1)-1;
                cr_h=min(cr_h,(v99)len);
                break;
            }
        }
    }
    if(sunno!=-1)
    {
        for(v99 i=sunno-1;i>=0;i--)
        {
            if(n[i]-'0'== 5 or n[i]-'0'==0)
            {
                v99 len=n.size()-(i+1)-1;
                cr_h=min(cr_h,(v99)len);
                break;
