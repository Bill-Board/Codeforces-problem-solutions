...
{
    /// code is here->
    v99 n,f=0,t=0,ans=7418880;
    vll a(8);
    for(v99 i=2;i<=5;i++)
    {
        cout<<"? 1 "<<i<<ses;cout.flush();
        read(a[i]);
        if(a[i]%15==0)f++;
        if(a[i]%23==0)t++;
    }
    ...
    cout<<"! "<<n<<whp;
    rep1(i,2,5)a[i]/=n,ans/=a[i],cout<<a[i]<<whp;
    cout<<ans/n<<ses;cout.flush();
}
