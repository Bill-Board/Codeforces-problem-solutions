...
bool accha(v99 b[])
{
    for(v99 i=1;i<=2*n;i++)if(b[i]!=i)return false;
    return true;
}
bool accha2(v99 b[])
{
    for(v99 i=1;i<=2*n;i++)if(b[i]!=a[i])return false;
    return true;
}

v99 alabu(bool ok,v99 b[])
{
    v99 cr_h=0;
    while(true)
    {
        if(ok)
        {
            for(v99 i=1;i<=2*n;i+=2)swap(b[i],b[i+1]);
            ok=false;
        }
        else
        {
            for(v99 i=1;i<=n;i+=2)swap(b[i],b[i+n]);
            ok=true;
        }
        cr_h++;
        if(accha(b))return cr_h;
        if(accha2(b))return LLONG_MAX;
    }
}
...
void solve()
{
    ...
    read(n);
    v99 b[2*n+3];
    rep1(i,1,2*n)cin>>a[i],b[i]=a[i];
    v99 ans=min(alabu(true,b),alabu(false,b));
