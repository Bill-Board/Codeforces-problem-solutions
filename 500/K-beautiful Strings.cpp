...
void solve()
{
    ...
    rep0(i,0,26)sum+=alabu(cnt[i],k);
    if(sum==0)return print(s);
    if(n%k)return print(-1);
    for(v99 i=n-1;~i;i--)
    {
        sum-=alabu(cnt[s[i]-'a'],k);
        cnt[s[i]-'a']--;
        sum+=alabu(cnt[s[i]-'a'],k);
        for(v99 j=s[i]-'a'+1;j<26;j++)
        {
            v99 sum_st=sum;
            sum_st-=alabu(cnt[j],k);
            cnt[j]++;
            sum_st+=alabu(cnt[j],k);
            if(i+sum_st<n)
            {
                string cr_h="";
                rep0(ii,0,26)cr_h+=string(alabu(cnt[ii],k),char(ii+'a'));
                cr_h+=string( max(0LL,n-(i+1+cr_h.size())),'a');
                sort(sob(cr_h));
                return print(s.substr(0,i)+char(j+'a')+cr_h);
            }
            cnt[j]--;
        }
    }
