...
///-------------------------------------------------------------------------------------------------------
v99 alabu(string s,char c)
{
    rep0(i,0,s.size())if(s[i]==c)return i;
}
...
void solve()
{
    ...
    do
    {
        ...
        {
            string pp=p[i];
            v99 pos1=alabu(s,pp[0]),pos2=alabu(s,pp[2]);
            if(pp[1]=='>' and pos1<pos2){ok=false;break;}
            if(pp[1]=='<'  and pos1>pos2){ok=false;break;}
        }
        if(ok)return print(s);
    }while(next_permutation(sob(s)));
    print("impossible");
}
