...
void solve()
{
    ...
    v99 n;read(n);
    vll a(n);read(a);
    vector<pair<v99,pii> >vp;
    for(v99 i=0;i<n;i+=2)
    {
        v99 ek=a[i],dui=a[i+1];
        for(v99 j=0;j<6;j++)
        {
            if(j%2==0)vp.pb({1,{i,i+1}}), a[i]+=a[i+1];
            else vp.pb({2,{i,i+1}}),a[i+1]-=a[i];
        }
    }
    cout<<vp.size()<<ses;
    for(auto i:vp)cout<<i.ff<<whp<<i.ss.ff+1<<whp<<i.ss.ss+1<<ses;
}
