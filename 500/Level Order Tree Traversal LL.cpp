...
void Insert(node *root,v99 val)
{
    print("YES");
    if(root->vall!=NULL)print(val,root->vall);
    if(root==NULL)
    {
        root =new node();
        root->vall=val;
    }
    else if(root->vall>val)print(root->vall),Insert(root->left,val);
