...
v99 alabu(char c)
{
    vector<v99>v;v99 cr_h=0,kot=0,inv=0;
    for(auto i:s)
    {
        v99 k=0;
        for(auto j:i)k+=(j==c);
        if(k>(i.size()-k))cr_h++,kot+=k,kot-=(i.size()-k);
        else v.pb(i.size()-k-k);
    }
    sort(sob(v));
    for(auto i:v)
    {
        if(kot-i>0)cr_h++,kot-=i;
        else break;
    }
    return cr_h;
}
...
{
    /// code is here->
    v99 n;read(n);s.resize(n);
    read(s);
    v99 cr_h=0;
    for(char c='a';c<='e';c++)cr_h=max(cr_h,alabu(c));
    print(cr_h);
}
