...
void update(v99 node,v99 val)
{
    ...
    if(t[node]>val)update(2*node,val);
    else update(2*node +1, val);
}
...
void solve(v99 tc)
{
    /// code is here->
    read(n);t.clear();
    rep0(i,0,n)read(a[i]),update(1LL,a[i]);
    queue<v99>q;q.push(1LL);
    vll ans;
    while(!q.empty())
    {
        v99 val=q.front();q.pop();
        if(t.find(val)==t.end())continue;
        ans.pb(t[val]);
        q.push(2*val);q.push(2*val +1);
    }
    cout<<"Case "<<tc<<":"<<ses;
