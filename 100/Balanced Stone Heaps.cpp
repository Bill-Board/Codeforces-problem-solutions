...
bool alabu(v99 val)
{
    //print("val ",val);
    pre.clear();
    vll b=a;pre.resize(n,0LL);
    for(v99 i=n-1;i>1;i--)
    {
        v99 mot=a[i]+pre[i];
        if(mot<val)return false;
        v99 kot=min(a[i],mot-val);
        v99 dif=kot/3;
        pre[i-1]+=dif;
        pre[i-2]+=2*dif;
        b[i]=mot-3*dif;
...
//    print(alabu(x));
    ...
    {
        v99 mid=(l+r)/2LL;
        if(alabu(mid))cr_h=max(mid,cr_h),l=mid+1;
        else r=mid-1;
    }
    print(max(m,cr_h));
}
