...
void solve()
{
    ...
    vector<pii>vp(k);
    map<v99,v99>mpp1,mpp2;
    for(auto &i:vp)read(i.ff),mpp1[i.ff]++;
    for(auto &i:vp)read(i.ss),mpp2[i.ss]++;
    sort(sob(vp));
    v99 sum=(k*(k-1))/2;
    v99 visi[max(a,b)+10]={};
    for(auto i:mpp1)sum-=(i.ss*(i.ss-1))/2;
    for(auto i:mpp2)sum-=(i.ss*(i.ss-1))/2;
    print(sum);
