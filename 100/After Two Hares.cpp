...
bool query_clm(v99 L,v99 R,v99 y)
{
    if(L>R)swap(L,R);
    return (c[R][y]-c[L-1][y])==0;
}
bool query_row(v99 L,v99 R,v99 x)
{
    if(L>R)swap(L,R);
    return (r[x][R]-r[x][L-1])==0;
}
...
void solve()
{
    ...
    while(q--)
    {
        v99 x1,x2,y1,y2;
        read(x1,y1,x2,y2);
        if(query_clm(x1,x2,y2) and query_row(y1,y2,x1))print("YES");
        else if(query_clm(x1,x2,y1) and query_row(y1,y2,x2))print("YES");
        else print("NO");
    }
}
