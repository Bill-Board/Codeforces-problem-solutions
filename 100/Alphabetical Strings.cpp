...
void solve()
{
    ...
    v99 n=s.size();
    string a,b;
    for(v99 i=0;i<n and s[i]!='a';i++)a+=s[i];
    for(v99 i=n-1;i>=0 and s[i]!='a';i--)b+=s[i];
    if(b.size()+a.size()+1!=n)return print("NO");
    for(char c='b';c<='z' and (a.size() or b.size());c++)
    {
        if(a.size() and a.back()==c)a.pop_back();
        else if(b.size() and b.back()==c)b.pop_back();
