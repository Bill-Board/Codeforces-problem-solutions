...
void out(priority_queue<v99,vector<v99> , greater<v99> >p)
{
    while(!p.empty())cout<<p.top()<<whp,p.pop();
    cout<<ses;
}
...
void solve()
{
    ...
    v99 n,m,k;cin>>n>>k>>m;
    double cr_h=0,sum=0;
    priority_queue<v99,vector<v99> , greater<v99> >q;
    rep0(i,0,n)
    {
        v99 x;cin>>x;
        q.push(x);
    ...
    {
        m=min(q.size()*k,m);
        double ans=(sum+m)/(double)q.size();
        //cout<<sum<<whp<<ans<<whp<<m<<ses;
        cr_h=max(cr_h,ans);
        sum-=q.top();
        q.pop();m--;
    }
    cout<<fixed<<setprecision(15)<<cr_h<<ses;
}
