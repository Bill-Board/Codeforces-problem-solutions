...
vll a,v1,v0;
v99 n,dp[5004][5004];
v99 alabu(v99 ind,v99 w)
{
    if(ind==v1.size()) return 0LL;
    if(w==v0.size()) return INF;
    v99 &cr_h=dp[ind][w];
    if(cr_h!=-1)return cr_h;
    return cr_h=min(abs(v1[ind]-v0[w])+alabu(ind+1,w+1),alabu(ind,w+1) );
}
