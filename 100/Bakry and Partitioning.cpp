...
v99 dfs(v99 ind , v99 xr)
{
    ...
    for(auto i:tom[ind])
    {
        if(visi[i]==0)x^=dfs(i,xr);
        if(xr==x)
        {
            tom[ind].erase(tom[ind].find(i));
            tom[i].erase(tom[i].find(ind));
            //print(i,ind);
            ok=true;
            return 1LL;
        }
...
{
    /// code is here->
    v99 n,k,xr=0;read(n,k);
    for(v99 i=0;i<=n;i++)tom[i].clear(),visi[i]=0;
    for(v99 i=1;i<=n;i++)read(a[i]),xr^=a[i];
    rep0(i,0,n-1)
    {
        v99 x,y;read(x,y);
        tom[x].insert(y);
    ...
    if(xr==0)return print("YES");
    if(k==2)return print("NO");
    v99 ans=0;
    while(ans<2)
    {
        ok=false;
