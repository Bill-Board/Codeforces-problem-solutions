...
void solve()
{
    ...
    map<v99,v99>mpp;
    rep0(i,1,n)mpp[a[i]-a[i-1]]++;
    if(mpp.size()>2)return print(0);
    set<v99>ans;
    if(mpp.size()==1)
    {
        if(n>2)
        {
            v99 val=a[1]-a[0];
            ans.insert(a[0]-val);ans.insert(a[n-1]+val);
        }
        else
        {
            v99 val=a[1]-a[0];
            ...
            {
                ans.insert(a[0]-val);
                ans.insert(a[n-1]+val);
            }
            else
            {
                ans.insert(a[0]-val);
                ans.insert(a[0]+ val/2);
                ans.insert(a[n-1]+val);
            }
        }
    }
    ...
    {
        auto i=mpp.begin(),j=prev(mpp.end());
        if(i->ff==0)return print(0);
        if(j->ss !=1 or j->ff&1 or j->ff/2 != i->ff )return print(0);
        for(v99 k=1;k<n;k++)
        {
            if(a[k]-a[k-1]==j->ff)
            {
                ans.insert(a[k-1]+i->ff);
                break;
            }
        }
    }
    ...
    if(ans.size())
    {
        for(v99 i:ans)cout<<i<<whp;cout<<ses;
    }
}
