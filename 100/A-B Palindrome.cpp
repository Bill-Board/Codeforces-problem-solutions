...
{
    /// code is here->
    v99 a,b,aa,bb;read(a,b);aa=a,bb=b;
    string s;read(s);
    v99 n=s.size();
    for(v99 i=0,j=n-1;i<j;j--,i++)
    {
        if(s[i]=='?' and s[j]=='?')continue;
        if(s[i]!='?' and s[j]!='?' and s[i]!=s[j])return print(-1);

        if(s[i]=='?')s[i]=s[j];
        else s[j]=s[i];
        if(s[i]=='0')a-=2;
        else b-=2;
        if(a<0 or b<0)return print(-1);
        //print(i,j,s);
    }
    //print(s);
    for(v99 i=0,j=n-1;i<j;j--,i++)
    ...
    if(a<0 or b<0)return print(-1);
    else if(one!=bb or z!=aa)return print(-1);
    print(s);
}
