...
{
    vll ans;
    rep0(i,pos,n)ans.pb(b[i]);
    rep0(i,0,pos)ans.pb(b[i]);
    rep0(i,0,n)ans[i]=(ans[i]+a[i])%n;
    return ans;
}
...
void solve()
{
    ...
    vll ans(n);
    vector<pii>vp;
    rep0(i,0,n)vp.pb({(a[0]+b[i])%n,i}),ans[i]=(a[i]+b[i])%n;
    sort(sob(vp));
    v99 visi[n+1]={};
    rep0(i,0,vp.size())
    {
        if(vp[0].ff==vp[i].ff and visi[vp[i].ss]==0)
        {
            visi[vp[i].ss]=1;
            vll p=alabu(vp[i].ss);
            if(p<ans)ans=p;
