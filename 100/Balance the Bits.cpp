...
bool chk(string s)
{
    ...
    {
        if(v.empty())v.pb(s[i]);
        else if(v.back()=='(' and s[i]==')')v.pop_back();
        else v.pb(s[i]);
    }
    return v.size()==0LL;
}
bool ok(string s,string a,string b)
{
    rep0(i,0,s.size())
    {
        if(s[i]=='0' and a[i]==b[i])return false;
        if(s[i]=='1' and a[i]!=b[i])return false;
    }
    return true;
}
...
{
    /// code is here->
    v99 n,cnt=0,k=1,flip=1;read(n);string s,a,b;read(s);
    for(auto i:s)cnt+=i=='1';
    if(n&1 or s[0]=='0' or s.back()=='0' or cnt&1)return print("NO");
    rep0(i,0,n)
    {
        if(s[i]=='1')
        {
