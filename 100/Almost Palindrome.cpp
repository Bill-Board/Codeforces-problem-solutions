...
{
    for(v99 i=0,j=s.size()-1;i<j;i++,j--)
...
void solve()
{
    ...
    string s;read(s);
    v99 n=s.size();
    if(is_pal(s))return print("YES");
    string a;
    for(v99 i=0,j=s.size()-1;i<j;i++,j--)
    {
        if(s[i]!=s[j])a.pb(s[i]),a.pb(s[j]);
    }
    if(a.size()>4)return print("NO");
    if(a.size()==2 and n%2==0)return print("NO");
    if(a.size()==2)
    {
        char c=s[n/2];
        if(c==a[0] or c==a[1]) print("YES");
        else print("NO");
        return;
    }
    else
    {
        if(a[0]==a[2] and a[1]==a[3])print("YES");
        else if(a[0]==a[3] and a[1]==a[2])print("YES");
        else print("NO");
    }
}
