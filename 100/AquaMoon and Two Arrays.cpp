...
void solve()
{
    ...
    for(auto &i:b)read(i),sum-=i;
    if(sum!=0)return print(-1);
    rep0(i,0,n)
    {
        if(a[i]==b[i])continue;
        v99 kot=b[i]-a[i];
        if(a[i]<b[i])/// ekhane j
        {
            for(v99 j=i+1;j<n and kot>0;j++)
            {
                if(a[j]>b[j])
                {
                    v99 ok=min(kot,a[j]-b[j]);
                    kot-=ok;
                    a[j]-=ok;
                    a[i]+=ok;
                    while(ok--)v.pb({j,i});
    ...
    print(v.size());
    if(v.size())
    for(auto i:v)print(i.ff+1,i.ss+1);
}
