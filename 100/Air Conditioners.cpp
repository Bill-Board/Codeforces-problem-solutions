...
///-------------------------------------------------------------------------------------------------------
v99 n,k;
vll alabu(priority_queue <pii, vector<pii>, greater<pii> > s)
{
    vll a(n);
    for(v99 i=0;i<n;)
    {
        if(s.empty()){a[i]=INF;i++;continue;}
        v99 val=s.top().ff-i,pos=s.top().ss;
        if(i>pos)s.pop();
        else a[i]=val,i++;
    }
    return a;
...
{
    /// code is here->
    read(n,k);vll p(k),a(n+1);
    read(p);
    priority_queue <pii, vector<pii>, greater<pii> > ek,dui;
    rep0(i,0,k)
    {
        v99 x;read(x);
        ek.push({x+p[i]-1,p[i]-1});
        dui.push({x+n-p[i],n-p[i]});
    }
    vll ans1=alabu(ek),ans2=alabu(dui);
    reverse(sob(ans2));
    rep0(i,0,n)cout<<min(ans1[i],ans2[i])<<whp;
    cout<<ses;
}
