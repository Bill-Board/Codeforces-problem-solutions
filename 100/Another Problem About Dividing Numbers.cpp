...
void alabu(v99 a)
{
    for(v99 i=0;i<pr.size();i++)
    {
        if(pr[i]*pr[i]>a)break;
        while(a%pr[i]==0)a/=pr[i],ek[pr[i]]++;
    }
    if(a>1)ek[a]++;
}
void alabu1(v99 a)
{
    ...
    {
        if(pr[i]*pr[i]>a)break;
        while(a%pr[i]==0)a/=pr[i],dui[pr[i]]++;
    }
    if(a>1)dui[a]++;
}
void solve()
{
    ...
    map<v99,v99>tin,fer;
    //print(ek);print(dui);
    v99 mxx=0,mnn=0,sum=0;
    for(auto i:ek)
    {
        sum+=i.ss;
        if(dui[i.ff]>=i.ss)continue;
        tin[i.ff]=i.ss-dui[i.ff];
    }
    for(auto i:dui)
    {
        sum+=i.ss;
        if(ek[i.ff]>=i.ss)continue;
        fer[i.ff]=i.ss-fer[i.ff];
    }
    if(a==b)
    {
        if(k==1) print("NO");
        else if(k<=sum)print("YES");
        else print("NO");
        return;
    }
    if(tin.empty() or fer.empty())mnn=1;
    else mnn=2;
    if(mnn<=k and k<=sum)print("YES");
    else print("NO");
}
