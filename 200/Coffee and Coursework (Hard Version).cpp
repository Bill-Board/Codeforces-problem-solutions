...
///-------------------------------------------------------------------------------------------------------
v99 n,m;
bool jdi_hoi(v99 i,vector<v99>a)
{
    v99 sum=0;
    for(v99 j=0;j<a.size();j++)sum+=max(0LL,a[j]-(j/i));
    return sum>=m;
}
...
void solve()
{
    ...
    {
        v99 mid=(r+l)>>1;
        if(jdi_hoi(mid,a))r=mid;
        else l=mid;
    }
    if(jdi_hoi(l,a))print(l);
    else if(jdi_hoi(r,a))print(r);
    else print(-1);
