...
///-------------------------------------------------------------------------------------------------------
vector<pair<v99,char> >a;
vll p;
void alabu(vector<pii>b)
{
    for(auto i:b)
    {
        //print(i.ff,i.ss);
        v99 x=i.ff,y=i.ss;
        char bx,by;
        if(x>0)p.pb(1LL), a.pb({abs(x),'R'}),bx='L';
        if(x<0)p.pb(1LL), a.pb({abs(x),'L'}),bx='R';
        if(y>0)p.pb(1LL), a.pb({abs(y),'U'}),by='D';
        if(y<0)p.pb(1LL), a.pb({abs(y),'D'}),by='U';
        p.pb(2LL);
        if(x)p.pb(1LL), a.pb({abs(x),bx});
        if(y)p.pb(1LL), a.pb({abs(y),by});
        p.pb(3LL);
    }
}
bool sort1(const pii &aa,const pii &bb)
{
    if(aa.ff==bb.ff)return aa.ss>bb.ss;
    return aa.ff<bb.ff;
}
bool sort2(const pii &aa,const pii &bb)
{
    if(aa.ff==bb.ff)return aa.ss<bb.ss;
    return aa.ff>bb.ff;
}
bool sort3(const pii &aa,const pii &bb)
{
    if(aa.ff==bb.ff)return aa.ss>bb.ss;
    return aa.ff>bb.ff;
}
void solve()
{
    ...
    for(auto i:p)
    {
        if(i==1)print(1,a.back().ff,a.back().ss),a.pop_back();
        else print(i);
    }
}
