...
void solve()
{
    ...
    v99 n;read(n);
    vll tom[n+2];
    v99 uni[n+1],a[n+1],ans[n+1]={};
    rep1(i,1,n)read(uni[i]);
    rep1(i,1,n)
    {
        read(a[i]);
        tom[uni[i]].pb(a[i]);
    ...
    rep1(i,1,n)
    {
        ...
        tom[i].pb(0LL);
        sort(sob(tom[i]));
        reverse(tom[i].begin()+1,tom[i].end());
        //print(tom[i]);
        for(v99 j=1;j<tom[i].size();j++)tom[i][j]+=tom[i][j-1];
        for(v99 j=1;j<tom[i].size();j++)
        {
            v99 sz=tom[i].size()-1;
            if(sz%j==0)ans[j]+=tom[i].back();
            else
            {
                v99 rem=sz%j;
                ans[j]+=tom[i][sz-rem];
    ...
    rep1(i,1,n)cout<<ans[i]<<whp ses[i==n];
