...
void acha(priority_queue<Node> a)
{
    while(!a.empty())print(a.top().cost,a.top().pos),a.pop();
    cout<<ses;
}
...
void solve()
{
    ...
    while(q--)
    {
        ...
        else if(x==2) /// sobar 1st r ta nibo polycarp
        {
            while( visi[vp.front().ss]!=0)vp.pop();
            visi[vp.front().ss]=1;
            cout<<vp.front().ss<<whp;
            vp.pop();
        }
        else ///je besi taka dise
        {
            while( visi[pq.top().pos]!=0)pq.pop();
            visi[pq.top().pos]=1;
            cout<<pq.top().pos<<whp;
            pq.pop();
        }
