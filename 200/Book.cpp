...
void solve()
{
    ...
    for(v99 i=1;i<=n;i++)if(kk[i].empty())s.insert({0LL,i});
    if(s.empty())return print(-1);
    v99 ans=0;
    while(s.size())
    {
        auto i=s.begin();
        v99 val=i->ff,ind=i->ss;
        s.erase(s.begin());
        print("val ind",ind,val);
        for(auto j:tom[ind])
        {
            v99 ek=p[j],dui=j;
            print("ekdui", ek, dui);
            auto u=s.find({ek,dui});
            if(u!=s.end())
                s.erase(u);
            if(ind>j)ek++;
            s.insert({ek,dui});
        ...
        for(auto j:s)cout<<j.ff<<whp<<j.ss<<ses;cout<<ses;
    }
    print(p);
    print(ans);
}
