...
inline void normal(v99 &a)
{
    a %= MOD;
    (a < 0) && (a += MOD);
}
inline v99 modMul(v99 a, v99 b)
{
    a %= MOD, b %= MOD;
    normal(a), normal(b);
    return (a*b)%MOD;
}
inline v99 modAdd(v99 a, v99 b)
{
    a %= MOD, b %= MOD;
    normal(a), normal(b);
    return (a+b)%MOD;
}
inline v99 modSub(v99 a, v99 b)
{
    a %= MOD, b %= MOD;
    normal(a), normal(b);
    a -= b;
...
inline v99 modInverse(v99 a)
{
    return modPow(a, MOD-2);    /// When MOD is prime.
}
inline v99 modDiv(v99 a, v99 b)
{
...
void solve()
{
    ...
    if(a<=b)
    {
        for(v99 i=1,j=1; j<=b; i+=2,j++)
            swap(v[i],v[i+1]);
        if(a==b and a!=0)swap(v[n],v[n-1]);
    }

    else
    {
        for(v99 i=n,j=1;j<=a; i-=2,j++)
            swap(v[i],v[i-1]);
        if(a==b and a!=0)swap(v[1],v[2]);
    }
    rep1(i,1,n)cout<<v[i]<<whp ses[i==n];
