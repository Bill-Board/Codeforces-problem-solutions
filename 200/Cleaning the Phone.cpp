...
///-------------------------------------------------------------------------------------------------------
void alabu(vll &aa,v99 m,v99 &sum)
{
        while(sum-aa.back()>=m)
        {
            sum-=aa.back();
...
{
    /// code is here->
    v99 n,m,s=0,sum=0,cr_h=0;read(n,m);
    vector<pii>vp(n);
    for(auto &i:vp)read(i.ff),s+=i.ff;
    for(auto &i:vp)read(i.ss),cr_h+=i.ss;
    ...
    sort(sob(a),greater<>());sort(sob(b));
    for(v99 ii=0;sum<m;)
    {
        v99 ek=0,dui=0,i=0;
        if(ii<a.size())i=a[ii];
        if(!b.empty())ek=b.back();
        if(b.size()>1)dui=b[b.size()-2];
        if(ek+sum>=m)bb.pb(ek),sum+=ek,b.pop_back();
        else if(i>ek+dui) aa.pb(i),ii++,sum+=i;
        else
        {
            if(ek)bb.pb(ek),b.pop_back();
            if(!b.empty())bb.pb(dui),b.pop_back();
            sum+=(ek+dui);
        }
    }
    if(!aa.empty())alabu(aa,m,sum);
    if(!bb.empty())alabu(bb,m,sum);
    print(aa.size()*2 + bb.size());
}
