...
v99 cnt[mxi],dp[mxi],a[mxi];

void solve()
{
    /// code is here->
    v99 n,cr_h=0;read(n);
    rep0(i,0,n)read(a[i]),cnt[a[i]]+=a[i];
    dp[1]=cnt[1];
    cr_h=cnt[1];
    for(v99 i=2;i<=100000;i++)/// hoito agertai othoba r ager value
        dp[i]=max(dp[i-1],dp[i-2]+cnt[i]),cr_h=max(cr_h,dp[i]);
    print(cr_h);
}
