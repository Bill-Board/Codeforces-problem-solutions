...
void solve()
{
    ...
    {
        v99 nij=i.ss;
        cr_h+=ncr(nij,3LL); /// 3 0 0
        auto j=mpp.find(i.ff+1);
        if(j!=mpp.end())
        {
            cr_h+=ncr(j->ss,2LL)*nij;  ///   1 2 0
            cr_h+=(ncr(nij,2LL) * j->ss);/// 2 1 0
        }
        auto k=mpp.find(i.ff+2);
        if(k!=mpp.end())
        {
            cr_h+=ncr(k->ss,2LL)*nij;     /// 1 0 2
            cr_h+=(ncr(nij,2LL) * k->ss);///  2 0 1
        }
        if(j!=mpp.end() and k!=mpp.end())cr_h+=(nij*k->ss*j->ss);/// 1 1 1
