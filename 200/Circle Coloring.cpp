...
{
    /// code is here->
    v99 n;read(n);vll a(n),b(n),c(n),p(n);
    ...
    if(ok(p[0],p[n-2],p[n-1]))print(p);
    else if(ok(p[0],p[n-2],a[n-1]))p[n-1]=a[n-1],print(p);
    else if(ok(p[0],p[n-2],b[n-1]))p[n-1]=b[n-1],print(p);
    else p[n-1]=c[n-1],print(p);
}
