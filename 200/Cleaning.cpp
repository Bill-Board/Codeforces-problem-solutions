...
///-------------------------------------------------------------------------------------------------------
map<pair<pii,bool>,v99>mpp;
v99 n,a[mxi];
v99 alabu(v99 ind,v99 last,bool ok)
{
    if(ind==n+1)return last==0;
    if(mpp.count({{ind,last},ok}))return mpp[{{ind,last},ok}];
    v99 cr_h=0;
    if(last<=a[ind])cr_h|=alabu(ind+1,a[ind]-last,ok);
    if(ok==0 and ind+1<=n and a[ind+1]>=last and a[ind]>=(a[ind+1]-last))
        cr_h|=alabu(ind+2,a[ind]-(a[ind+1]-last),1);
    return mpp[{{ind,last},ok}]=cr_h;
}
...
void solve()
{
    ...
    read(n);mpp.clear();
    rep1(i,1,n)read(a[i]);
    if(alabu(1LL,0LL,0)==1)print("YES");
    else print("NO");
}
