...
void solve()
{
    ...
    for(akib i:vp)print(i.x,i.y,i.ind);
    set<v99>p,pp;
    v99 ans[n+2],ek=0,dui=0;
    while(p.size()<=1 and ek<n)p.insert(vp[ek++].x);
    while(pp.size()<=1 and dui<n)pp.insert(vp[dui++].y);
    v99 pos=max(ek-1,dui-1),val=vp[0].ind;//print(ek-1,dui-1);
    rep0(i,0,pos)ans[vp[i].ind]=-1;
    rep0(i,pos,n)
    {
        ans[vp[i].ind]=val;
    }
