...
void solve()
{
    ...
    vll a,b,c,d;
    rep0(i,0,n)
        if(s[i]=='0' and t[i]=='0')a.pb(i);
        else if(s[i]=='0' and t[i]=='1')b.pb(i);
        else if(s[i]=='1' and t[i]=='0')c.pb(i);
        else if(s[i]=='1' and t[i]=='1')d.pb(i);
    rep1(i,0,c.size())
    {
        rep1(j,0,d.size())/// one nilam i+j
        {
            v99 one_lagbe= b.size() + (d.size()-j)-(i+j);
            if(one_lagbe>=0 and one_lagbe<=b.size())
            {
                v99 kot=n/2 -(i+j+one_lagbe);
                if(kot>=0 and kot<=a.size())
                {
                    rep0(z,0,i)cout<<c[z]+1<<whp;
                    rep0(z,0,j)cout<<d[z]+1<<whp;
                    rep0(z,0,one_lagbe)cout<<b[z]+1<<whp;
                    rep0(z,0,kot)cout<<a[z]+1<<whp;
                    return;
                }
            }
        }
