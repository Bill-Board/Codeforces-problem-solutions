...
{
    /// code is here->
    v99 n,k,cr_h=-LLONG_MAX;read(n,k);
    vll p,a(n+1),tom[n+2];
    rep1(i,1,n)
    {
        read(a[i]);
        if(tom[a[i]].empty())p.pb(a[i]);
        tom[a[i]].pb(i);
    }
    sort(sob(p));
    rep1(i,1,n-1)cr_h=max(cr_h,(i*(i+1)) - (k*(a[i]|a[i+1])));
    rep0(i,0,p.size())
    {
        v99 last=tom[p[i]].back();tom[p[i]].pop_back();
        if(tom[p[i]].size())cr_h=max(cr_h,(last*tom[p[i]].back())-(k*p[i]));
        if(i+1<p.size())cr_h=max(cr_h,(last*tom[p[i+1]].back())-(k*(p[i]|p[i+1])));
    }
    for(v99 i=1;i<n;i++)
    {
        for(v99 j=i+1;j<=min(n,i+k);j++)
            cr_h=max(cr_h,i*j-k*(a[i]|a[j]));
    }
    print(cr_h);
}
