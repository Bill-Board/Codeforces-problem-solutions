...
void solve()
{
    ...
    rep0(i,0,n)
    {
        v99 x;read(x);
        v99 upore=x+k+k-1;
        if(i==0 or i==n-1)vp.pb({x,x+k});
        else vp.pb({x,upore});
    }
    //for(auto i:vp)print(i.ff,i.ss);cout<<ses;
    v99 x=vp[0].ff,y=vp[0].ss;
    for(v99 i=1;i<n;i++)
    {
        if(y<=vp[i].ff or x>=vp[i].ss){print("NO");return;}
        //if(x<=vp[i].ff and y<=vp[i].ss)y=y+k-1,x=vp[i].ff;/// incresing
        //else if(x>=vp[i].ff and y>=vp[i].ss ) x=x-k+1,y=vp[i].ss;
        //x=max(0LL,x);
        //print(x,y);
        x = max(vp[i].ff, x + 1 - k);
		y = min(vp[i].ss, y - 1 + k);
    ...
    if(y<=vp[n-1].ff or x>=vp[n-1].ss)print("NO");
    else print("YES");
}
