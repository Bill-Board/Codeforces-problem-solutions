...
void solve()
{
    /// code is here->
    v99 n;read(n);
    string s=to_string(n);reverse(sob(s));
    vll a(s.size(),0);
    set<pii>p;
    for(v99 i=0;i<s.size();i++)
    {
        if(i+2<s.size())
        {
            v99 kot=(s[i]-'0')+10 - 9 ;print(kot);
            for(v99 j=kot,k=9;j<=9;j++,k--)p.insert({j,k});
            for(v99 j=1,k=s[i]-'0'-1;k>=0;j++,k--)p.insert({j,k});
            a[i+2]=1;
            if(a[i]==1)
            {
                s[i]=s[i]=='0'?'9':s[i]-1;
