...
v99 alabu(string s,v99 flag)
{
    v99 sum=0;
    for(v99 i=1;i<=flag;i++)
    {
...
v99 ok(v99 n)
{
    v99 sum4=(n/4),sum100=(n/100),sum400=(n/400);
    v99 lf=((sum4-sum100+sum400)+mod)%mod;
    return max(0LL,lf);
}
...
void solve()
{
    /// code is here->
    v99 a,b;read(a,b);
    v99 no_lf=0,yes_lf=0;
    v99 no[]={0,31,28,31,30,31,30,31,31,30,31,30,31};
    v99 yes[]={0,31,29,31,30,31,30,31,31,30,31,30,31};
    for(v99 i=1;i<=12;i++)no_lf+=alabu(to_string(i),no[i]);
    for(v99 i=1;i<=12;i++)yes_lf+=alabu(to_string(i),yes[i]);
    v99 cr_h=0;
    no_lf%=mod;yes_lf%=mod;
    if(a==0)cr_h+=yes_lf,a++;
    if(a>b)return print(cr_h%mod);
    v99 lfyr=((ok(b)-ok(a-1))+mod)%mod;
    v99 notlfyr=(((b-a+1)%mod -lfyr)+mod)%mod;
    v99 ans=(( lfyr*yes_lf )%mod + ( notlfyr*no_lf ))%mod;
    print((ans+cr_h)%mod);
