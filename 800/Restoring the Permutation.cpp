...
///-------------------------------------------------------------------------------------------------------
v99 n,a[mxi];
void minimal(set<v99>s)
{
    vll b;
    rep1(i,1,n)
    {
        if(s.find(a[i])!=s.end())b.pb(a[i]),s.erase(s.find(a[i]));
        else
        {
            v99 val=*s.begin();
            b.pb(val);
...
void mxmal(set<v99>s)
{
    set<v99>k;
    v99 b[n+5]={};
    rep1(i,1,n)if(k.find(a[i])==k.end())s.erase(s.find(a[i])),k.insert(a[i]),b[i]=a[i];
    //for(auto i:s)cout<<i<<whp;cout<<ses;
    rep1(i,1,n)if(b[i]==0)
    {
    ...
    rep1(i,1,n)cout<<b[i]<<whp ses[i==n];
}
