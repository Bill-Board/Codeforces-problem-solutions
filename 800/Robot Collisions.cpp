...
bool sortval(const akib &a,const akib &b)
{
    return a.pos<b.pos;
}
v99 ans[2*mxi];
void alabu(vector<akib> a,v99 m)
{
    vector<pii>p,n;
    for(auto i:a)
    {
        ...
        else /// negetive
        {
            ...
            {
                v99 dif=abs(n.back().ff-i.pos);
                v99 kot=n.back().ff + dif/2 ;
                ans[i.ind]=kot;
                ans[n.back().ss]=kot;
                n.pop_back();
            }
            else n.pb({i.pos,i.ind});
        }
    }
    while(p.size()>1)
    {
        pii boro=p.back();p.pop_back();
        pii choto=p.back();p.pop_back();

        v99 dif=(m-boro.ff),nop=choto.ff+dif;
        v99 kot=dif+( abs(nop-m)/2 );
        ans[boro.ss]=kot;
        ans[choto.ss]=kot;
    }
    if(p.size() and n.size())
    {
        ...
        if(p.back().ff+sec<=m)
        {
            pii boro=mp(p.back().ff+sec,p.back().ss);
            pii choto=mp(0LL,n.back().ss);
            v99 dif=(m-boro.ff),nop=choto.ff+dif;
            kot+=dif+( abs(nop-m)/2 );
            ans[boro.ss]=kot;
            ans[choto.ss]=kot;
...
void solve()
{
    ...
    v99 n,m;read(n,m);
    vector<akib>a(n);
    for(v99 i=0;i<n;i++)read(a[i].pos),a[i].ind=i;
    for(v99 i=0;i<n;i++)
    {
        char c;read(c);
        if(c=='L')a[i].di=-1;
    ...
    sort(sob(a),sortval);
    vector<akib>even,odd;
    for(v99 i=0;i<n;i++)
    {
        if(a[i].pos&1)odd.pb(a[i]);
        else even.pb(a[i]);
    }
    rep0(i,0,n)ans[i]=-1;
    alabu(even,m);
    alabu(odd,m);
    rep0(i,0,n)cout<<ans[i]<<whp ses[i==n-1];
