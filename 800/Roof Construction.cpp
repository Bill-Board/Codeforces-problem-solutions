...
///-------------------------------------------------------------------------------------------------------
v99 alabu(v99 kot,v99 off)
{
    v99 ulta=0,gun=1;
    while(kot>0)
    {
        if(kot%2==0 or gun==off)ulta+=gun;
        kot/=2;
        gun*=2;
    }
    return ulta;
...
{
    /// code is here->
    v99 n,k=LLONG_MAX;read(n);
    v99 l=n;
    while(!isPowerOfTwo(l))l-=(l&-l);
    if(isPowerOfTwo(n))l/=2;
    vll a;
    set<v99>s;

    for(v99 i=l+1;i<n;i++)a.pb(i),s.insert(i);
    if(s.find(l)==s.end() and l<n)a.pb(l),s.insert(l);
    for(v99 i=0;i<n;i++)
    {
        if(s.find(i)==s.end())a.pb(i),s.insert(i);
    }
    print(a);
}
