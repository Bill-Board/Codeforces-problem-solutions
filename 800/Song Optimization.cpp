...
void solve()
{
    ...
    rep0(p,0,k)
    {
        rep0(i,0,k)
        {
            rep0(j,0,k)fld[i][j]=min(fld[i][j],fld[i][p]+fld[p][j]);
        }
    }
    vll a(n+3);
    v99 sum=0;
    rep1(i,1,n)
    {
        read(a[i]);
        a[i]--;
        if(i>1)sum+=fld[a[i-1]][a[i]];
    }
    v99 cr_h=LLONG_MAX;
    rep1(i,1,n)
    {
        v99 kot=0;
        if(i>1)kot+=fld[a[i-1]][a[i]];
        if(i<n)kot+=fld[a[i]][a[i+1]];
//        for(v99 j=0;j<k;j++)
//        {
//            v99 ok=0;
//            if(i>1)ok+=fld[a[i-1]][j];
//            if(i<n)ok+=fld[j][a[i+1]];
//            cr_h=min(cr_h,sum-kot+ok);
//        }
    v99 ans=0;
    if(i>1 and i<n)ans=fld[a[i-1]][a[i+1]];
    cr_h=min(cr_h,sum-kot+ans);
    }
    print(cr_h);
}
