...
vll a,pre;
v99 n,k,sum;
bool ok(v99 steps,v99 baki)
{
    v99 m=min(n-1,baki);
    steps+=baki-m;
    v99 new_sum=sum-steps - pre[m] +( (a[n]-steps)*m);
    //print("new sum ",steps,baki,new_sum,ses,ses,ses);
    return new_sum<=k;
}
...
bool alabu(v99 step)
{
    ...
    //print("step ",step);
   for(v99 i=0;i<min(n,step);i++)
   {
       v99 baki=step-i;
       v99 new_last=a[n]-baki;
       v99 new_sum=sum-baki-pre[i]+(new_last*i);
       if(new_sum<=k)return true;
   }
   return false;
}
...
void solve()
{
    ...
    read(n,k);
    a.clear();pre.clear();
    a.resize(n+1),pre.resize(n+2);
    sum=0;
    rep1(i,1,n)read(a[i]);
    if(n==1)return print(max(0LL,a[1]-k));
    sort(a.begin()+1,a.end(),greater<>());
    rep1(i,1,n)pre[i]+=pre[i-1]+a[i];
    sum=pre[n];
    //if(sum<=k)return print(0);
    v99 l=0,r=9e18,ans=0;
    while(l<=r)
    {
        v99 mid=(l+r)/2;
        //print("1st ", l,mid,r,ses);
        if(alabu(mid))ans=mid,r=mid-1;
        else l=mid+1;
    }
    print(ans);
}
