...
v99 alabu(v99 a,v99 b)
{
    ...
    vll v=ita;
    v99 last=-1;
    for(v99 i=ita.size()-1;i>=0;i--)
    {
        if(ita[i]==1 and itb[i]==0)
        {
            v[last]=1;
            for(v99 j=last-1;j>=0;j--)v[j]=0;
           break;
        }
        if(ita[i]==0 and itb[i]==1)last=i;
    }
    //print(aa,bb);
    //print(ita);print(itb);print(v);
    v99 kot=okay(v)-okay(ita);
    //print(okay(v),okay(ita));
    if(kot+aa!=bb)kot++;
    return kot;
...
void solve()
{
    /// code is here->
    v99 a,b;read(a,b);
    v99 last=b,cr_h=LLONG_MAX;
    if(isPowerOfTwo(last))last++;
    while(!isPowerOfTwo(last))last+=(last & -last);
    for(v99 i=b;i<last;i++)
    {
        cr_h=min(cr_h,alabu(a,i)+i-b);
    }
    print(cr_h);
}
