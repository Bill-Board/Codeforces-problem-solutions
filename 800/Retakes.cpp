...
///-------------------------------------------------------------------------------------------------------
void ok(priority_queue<pii>q)
{
    while(q.size())cout<<q.top().ff<<whp <<q.top().ss<<ses ,q.pop();
    cout<<ses ses;
}
...
void solve()
{
    ...
    for(v99 i=1,cnt=0;cnt<n and i<=2000000;i++)
    {
        for(auto j:a[i])q.push(j),cnt++;
        //ok(q);
        if(q.empty())continue;
        ans[q.top().ss]=i;
        q.pop();
        while(q.size() and cnt==n)ans[q.top().ss]=++i,q.pop();
    }
