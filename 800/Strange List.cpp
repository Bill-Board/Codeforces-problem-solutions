...
v99 alabu(v99 a,v99 x,v99 &miin,vll &b)
{
    v99 h=1;
    while(a%x==0)
    {
...
{
    /// code is here->
    v99 n,x,cr_h=0,miin=LLONG_MAX;read(n,x);
    vll a(n),b;
    for(auto &i:a)
    {
        cin>>i;
        cr_h+=alabu(i,x,miin,b);
    }
    //print(b);print(miin);
    bool paichi=false;
    for(v99 i=0;i<n;i++)
    {
        if(b[i]==miin)paichi=true;
        if(paichi)cr_h+=(miin*a[i]);
        else cr_h+=(miin+1)*a[i];
    }print(cr_h);
}
