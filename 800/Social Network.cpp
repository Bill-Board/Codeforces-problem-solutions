...
void solve()
{
    ...
    rep1(i,1,d)
    {
        ...
        kot+=dsu(x,y);
        set<v99>s;
        rep1(j,1,n)s.insert(root(j));
        priority_queue<v99>q;
        for(auto j:s)q.push(sze[j]);
        v99 m=0;
        for(v99 j=0;j<=kot and q.size();j++)m+=q.top(),q.pop();
        ans.pb(m-1);
    }
