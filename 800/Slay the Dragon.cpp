...
{
    v99 ekhn =sum-val;
    v99 ans=max(0LL,x-val)+max(0LL,y-ekhn);
    return ans;
}
...
void solve()
{
    ...
    v99 n,m;read(n);
    set<v99>a;
    rep0(i,0,n)cin>>x,a.insert(x),sum+=x;
    read(m);
    //print(a);
    while(m--)
    {
        v99 ans=LLONG_MAX;read(x,y);
        auto l=a.lower_bound(x),u=a.upper_bound(x);
        for(v99 i=0;i<3 and l!=a.begin();i++,l--);
        for(v99 i=0;i<3 and u!=a.end();i++,u++);
        while(l!=u)
        {
            ans=min(ans,alabu(*l));
            l++;
