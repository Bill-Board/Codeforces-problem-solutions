...
void sieve()
{
    /// effictive way to find all sum of divisors
    /**for(v99 i=1;i<N;i++)
    {
        for(v99 j=1;i*j<N;j++)a[i*j]+=i;
    }
    */
    /// another effictive way (1+p^1+p^2+p^3) all possible case ber korbo
    fill(a,a+N,1);
    for(v99 i=2;i<N;i++)
    {
        if(a[i]!=1)continue;
    ...
    for(v99 i=1;i<N;i++)if(a[i]<N and ans[a[i]]==0)ans[a[i]]=i;
}
