...
void solve()
{
    ...
    for(auto i:mpp)
    {
        if(i.ff<1)koita+=i.ss,p[i.ff]++,dis++;
    }
    v99 cr_h=0;
    for(auto i:mpp)cr_h=max(cr_h,i.ss);
    if(dis<koita)return print(max(cr_h,koita));
    v99 dif=LLONG_MAX;
    for(v99 i=1;i<a.size();i++)dif=min(dif,abs(a[i]-a[i-1]));
    for(auto i:mpp)
    {
        if(i.ff>0 and dif>=i.ff)return print(max(koita+1,cr_h));
    }
    print(max(cr_h,koita));
}
