...
void solve()
{
    ...
    rep1(i,1,n)
    {
        ...
        else if(tom[i].size()>1)
        {
            ans[tom[i][1]]=i;
            s.erase(s.find(i));
        }
    }//print(ans);
    vll a,b,c;// a =val,b=pos
    for(v99 i:s)a.pb(i);
    for(v99 i=1;i<=n;i++)if(ans[i]==0)b.pb(i);
    for(auto i:b)if(s.find(i)!=s.end())c.pb(i),s.erase(s.find(i));
//    print(ans);
//    print(c);
    //for(v99 i:s)cout<<i<<whp;cout<<ses;
    if(c.size()>1){
        for(v99 i=0;i<c.size()-1;i++)
        {
            ans[c[i]]=c[i+1];
            if(i==c.size()-2)ans[c[i+1]]=c[0];
            //print(c[i+1],c[i]);
        }
        //print(ans);
        for(v99 i=1;i<=n;i++)if(ans[i]==0)
            {
                ans[i]=*s.begin();
                s.erase(s.begin());
            }
    ...
    else if(c.size()==1)
    {
        ...
        if(s.empty())
        {
            for(v99 i=1;i<=n;i++)
            {
                if(i==c.back())continue;
                if(ans[i]==e[c.back()])
                {
                    swap(ans[i],ans[c.back()]);
                    break;
                }
            }
        }
        else
        {
            ...
            swap(ans[c.back()],ans[pos]);
        }
    }
    else
    {
        for(v99 i=0;i<b.size();i++)ans[b[i]]=a[i];
    }
    v99 cr_h=0;
    rep1(i,1,n)cr_h+=(ans[i]==e[i]);
    print(cr_h);
    rep1(i,1,n)cout<<ans[i]<<whp;cout<<ses;
}
