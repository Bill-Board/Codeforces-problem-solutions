...
///-------------------------------------------------------------------------------------------------------
v99 alabu(string s,v99 n)
{
    v99 pos=0,mn=0,mx=0;
    for(auto i:s)
    {
        v99 dif=abs(mn-mx)+1;
        if((i=='L' or i=='U') and (abs(mx-mn+1)+1)<=n)pos--;
        else if((i=='R' or i=='D') and (abs(mx-mn+1)+1)<=n)pos++;
        mn=min(pos,mn);
        mx=max(mx,pos);
    }
    //print(mn,mx);
...
void solve()
{
    ...
    v99 n,m;read(n,m);
    string s,l,u;read(s);
    v99 kotl=0,kotr=0,kotu=0,kotd=0;
    for(auto i:s)
    {
        if(i=='L' or i=='R')
        {
    ...
    //print(l,u);
    v99 ansx=1,ansy=1;
    if(u.size())ansx=alabu(u,n);
    if(l.size())ansy=alabu(l,m);
    print(ansx,ansy);
}
