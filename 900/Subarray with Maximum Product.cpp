...
///-------------------------------------------------------------------------------------------------------
v99 n,x,a[mxi],inv[103],pre[mxi];
void mod_inv(v99 mod)
{
    for(v99 i=0;i<=100;i++)inv[i]=modularInverse(i,mod);
    pre[0]=a[0]%mod;
    rep0(i,1,n)pre[i]=(pre[i-1]*a[i])%mod;
}

pii alabu(pii ans,v99 x,v99 y)
{
    if(x>ans.ff)ans=pii(x,y);
    else if(x==ans.ff)ans=pii(x,min(ans.ss,y));
    return ans;
}
...
void solve()
{
    /// code is here->
    read(n,x);
    unordered_map<v99,v99>mpp;
    rep0(i,0,n)read(a[i]);
    mod_inv(x);
    pii ans(0LL,n);
    for(v99 i=0;i<n;i++)
    {
        ans=alabu(ans,pre[i],i+1);
        for(auto j:mpp) ans=alabu(ans,(pre[i]*j.ff)%x,i-j.ss);
        mpp[inv[pre[i]]]=i;
    }
    print(ans.ff,ans.ss);
}
