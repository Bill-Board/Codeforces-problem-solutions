...
///-------------------------------------------------------------------------------------------------------
v99 area(v99 x1,v99 y1,v99 x2,v99 y2,v99 x3,v99 y3)
{
    v99 ans=(x1*(y2-y3)) +(x2*(y3-y1))+(x3*(y1-y2));
    //v99 ans=x1 *(y2 – y3 ) + x2 *(y3 – y1 ) + x3 *(y1 – y2);
    return abs(ans);
}
...
void solve()
{
    ...
    rep0(i,0,4)sort(sob(tom[i]));
    v99 cr_h=0;
    rep0(i,0,4)
    {
        v99 x1=tom[i].front().ff,y1=tom[i].front().ss;
        v99 x2=tom[i].back().ff,y2=tom[i].back().ss;
        rep0(j,0,4)
        {
            if(j==i)continue;
            for(auto k:tom[j])
            {
                v99 x3=k.ff,y3=k.ss;
                v99 ans=area(x1,y1,x2,y2,x3,y3);
                cr_h=max(cr_h,ans);
            }
        }
    }
