...
v99 ok(v99 x,v99 yl,v99 yr)
{
    v99 cnt=0;//print("x y",x,yl,yr);
    bool ki=false;
    while(c[x][yl]=='*' and c[x][yr]=='*')
    {
        cnt++;
        x--;
        yl--;
        yr++;
        if(x<0 or yl<0 or yr>=m)break;
    }
    return cnt;
}

void ppp(v99 x,v99 yl,v99 yr,v99 cnt)
{
    while(c[x][yl]=='*' and c[x][yr]=='*')
    {
        visi[x][yl]=max(visi[x][yl],cnt);
        visi[x][yr]=max(visi[x][yr],cnt);
        x--;
        yl--;
        yr++;
        if(x<0 or yl<0 or yr>=m)break;
...
void solve()
{
    ...
    v99 ans=LLONG_MAX;
    for(v99 i=0;i<n;i++)
        for(v99 j=0;j<m;j++)read(c[i][j]),visi[i][j]=0LL;
    for(v99 i=n-1;i>0;i--)
    {
        for(v99 j=0;j<m;j++)
        {
            if(c[i][j]=='*' and j-1>=0 and j+1<m )
            {
                v99 cnt=ok(i-1,j-1,j+1);
                if(cnt>0)ppp(i,j,j,cnt);
    ...
    for(v99 i=0;i<n;i++)
    {
        for(v99 j=0;j<m;j++)
        {
            if(c[i][j]=='*' and visi[i][j]==0)
            {
                return print("NO");
            }
            if(visi[i][j]!=0)ans=min(ans,visi[i][j]);
        }

    }
    if(ans<k)return print("NO");
    print("YES");
}
