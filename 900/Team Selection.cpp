...
///-------------------------------------------------------------------------------------------------------
const v99 N=1000;
v99 n,k,dp[N+2][N+2],a[N+2],b[N+2];

v99 solve(v99 ind,v99 last)
{
    ...
    if(last>k)return -LLONG_MAX;
    if(dp[ind][last]!=0)return dp[ind][last];
    v99 kot=-LLONG_MAX;
    kot=max(kot,a[ind]*b[last]+solve(ind+1,last+1));
    kot=max(kot,solve(ind+1,last));
    return dp[ind][last]=kot;
}
