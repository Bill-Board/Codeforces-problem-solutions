...
vll tom[N];
v99 val[N],ans[N];
map<v99,vector<v99> >mpp;
v99 dfs(v99 ind,v99 par)
{
    set<v99> a;
    for(auto i:tom[ind])
    {
    ...
    for(auto i=mpp.rbegin();i!=mpp.rend();i++)
    {
        if(a.find(i->ff)==a.end())
        {
            ...
            ans[ind]=p;
            (j->ss).pop_back();
            if(mpp[kot].empty())mpp.erase(mpp.find(kot));
            break;
        }
    }
    return val[p];
...
signed main()
{
    for(v99 i=1,j=1;i<mxi;i++)
    {
        if(isPowerOfTwo(i))val[i]=j++;
        else val[i]=val[i-1];
    }
    //dua
