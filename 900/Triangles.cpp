...
///-------------------------------------------------------------------------------------------------------
v99 n;
v99 ber_kor_row(vector<pii>vp,v99 top_val)
{
    v99 cr_h=-1;
    for(auto i:vp)cr_h=max(cr_h,abs(top_val-i.ff)*max(i.ss,n-1-i.ss));
    return cr_h;
}

v99 ber_kor_clm(vector<pii>vp,v99 top_val)
{
    v99 cr_h=-1;
    for(auto i:vp)cr_h=max(cr_h,max(i.ff,n-1-i.ff)*abs(i.ss-top_val));
    return cr_h;
}
...
void solve()
{
    ...
    rep0(i,0,10)
    {
        if(tom[i].size()<2)continue;
        v99 cr_h=0;
        vector<v99>r,c;
        for(auto j:tom[i])r.pb(j.ff),c.pb(j.ss);
        sort(sob(r));sort(sob(c));
        cr_h=max({cr_h,ber_kor_row(tom[i],r.front()),ber_kor_row(tom[i],r.back())});
        cr_h=max({cr_h,ber_kor_clm(tom[i],c.front()),ber_kor_clm(tom[i],c.back())});
        ans[i]=cr_h;
    }
