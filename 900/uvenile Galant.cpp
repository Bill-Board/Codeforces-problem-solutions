...
///-------------------------------------------------------------------------------------------------------
v99 fact[mxi*6],inv[mxi*6],mod=998244353LL;
void factoria()
{
    fact[0]=inv[0]=1LL;
    for(v99 i=1;i<=1000002;i++)fact[i]=(fact[i-1]*i)%mod;
    for(v99 i=1;i<=1000002;i++)inv[i]=bigmod(fact[i],mod-2,mod);
}
v99 ncr(v99 n,v99 r)
{
    if(n<r or n<0 or r<0)return 0LL;
    v99 kot=(fact[n]*inv[n-r])%mod;
    return (inv[r]*kot)%mod;
}

void solve()
{
    /// code is here->
    v99 n;read(n);factoria();
    v99 space=n/2,one=(n%2!=0),ans=0;
    while(space>one)
    {
        print(space,one,ncr(space,one)*fact[space/2]);
        ans+=ncr(space,one)*fact[space/2];
        ans%=mod;
        one+=2;
        --space;
    }
