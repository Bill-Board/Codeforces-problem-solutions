...
void solve()
{
    ...
    for(auto i:vp)
    {
        v99 obsx=i.ff,obsy=i.ss,x=0,y=0;
        for(v99 i=0;i<s.size();i++)
        {
            if(s[i]=='L' and (  obsx!=(x-1) or obsy!=y ))--x;
            if(s[i]=='R' and (  obsx!=(x+1) or obsy!=y ))++x;
            if(s[i]=='U' and (  obsx!=(x) or obsy!=y+1 ))++y;
            if(s[i]=='D' and (  obsx!=(x) or obsy!=y-1 ))--y;
        }
        if(x==0 and y==0){print(obsx,obsy);return;}
    }
    print("0 0");
}
