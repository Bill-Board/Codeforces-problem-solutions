...
///-------------------------------------------------------------------------------------------------------
vll tom[N];
v99 n,m,mod,ache[N][N];

v99 alabu(v99 s,v99 d)
{
    v99 cr_h=0;
    if(tom[s].empty())return cr_h;
    for(v99 i:tom[s])cr_h+=ache[i][d];
    return (cr_h*(cr_h-1))/2;
}
...
set<v99> bfs(v99 ind)
{
    ...
    while(q.size())
    {
        v99 p=q.front();q.pop();a.insert(p);
        if(!cnt)continue;
        for(auto i:tom[p])q.push(i);
        cnt--;
    }
    a.erase(a.find(ind));
    return a;
