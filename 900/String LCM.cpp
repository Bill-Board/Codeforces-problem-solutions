...
///-------------------------------------------------------------------------------------------------------
string ss="";
string alabu(string s,string t)
{
    v99 sn=s.size(),tn=t.size();
    string ans="";
    for(v99 i=0;;i++)
    {
        ans+=t[i%tn];
        if(t[i%tn]!=s[i%sn])return "-1";
        if((i+1)%sn==0 and (i+1)%tn==0) return ans;
    }
    return ans;
}
...
void solve()
{
    ...
    string s,t;read(s,t);
    //if(s.size()<t.size())swap(s,t);/// s alws boro
    print(alabu(s,t));
}
