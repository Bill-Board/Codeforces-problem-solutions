...
///-------------------------------------------------------------------------------------------------------
v99 a[2003][2003],n,m,w,visi[2003][2003];

v99 bfs(v99 indx,v99 indy)
{
    v99 cr_h=LLONG_MAX;
    rep1(i,0,n)rep1(j,0,m)visi[i][j]=-1;
    queue<pii>q;
    q.push({indx,indy});
    visi[indx][indy]=0;
    while(!q.empty())
    {
        v99 xx=q.front().ff,yy=q.front().ss;
        q.pop();
        if(a[xx][yy]>0)cr_h=min(cr_h,a[xx][yy]+visi[xx][yy]);
        for(v99 i=0;i<4;i++)
        {
            v99 x=fx[i]+xx,y=yy+fy[i];
            if(x>0 and x<=n and y>0 and y<=m and visi[x][y]==-1 and a[x][y]!=-1)
            {
                visi[x][y]=visi[xx][yy]+w;
                q.push({x,y});
            }
        }
    }
...
void solve()
{
    /// code is here->
    read(n,m,w);
    rep1(i,1,n)rep1(j,1,m)read(a[i][j]);
    v99 home=bfs(1LL,1LL),school=bfs(n,m);
    if(visi[1][1]==-1) /// amake sure portal path usa kora lagbe
    {
        /// 2 tar jekono ektay portal nai ,tar mane ami jete parbona
        if(home==LLONG_MAX or school==LLONG_MAX)print(-1);
        /// amake sure portal use kora lagbe
        else print(home+school);
    }
    else/// amar direct path ache home theke school a
    {
        if(home==LLONG_MAX or school==LLONG_MAX)print(visi[1][1]);
        else print(min(visi[1][1],home+school));
    }
