...
bool ache(v99 x11, v99 y11, v99 x22,
               v99 y22, v99 x, v99 y)
{
    if (x >= x11 and x <= x22 and y >= y11 and y <= y22)
        return true;

    return false;
}
bool alabu(v99 a,v99 b,v99 c,v99 d)
{
    bool ki=false;
    ki|=ache(a,b,c,d,x1,y1);
    ki|=ache(a,b,c,d,x2,y2);
    ki|=ache(a,b,c,d,x1,y2);
    ki|=ache(a,b,c,d,x2,y1);
    return !ki;
}
...
void solve()
{
    ...
    double ans=INF,k=0.0;//print(x2-x1+w,y2-y1+h);
    //if(x2-x1+w>W and y2-y1+h>H)return print(-1);

    /// left up
    v99 a=0,b=(H-h),c=w,d=H;
    //print(a,b,c,d);
    if(alabu(a,b,c,d))ans=min(ans,k);//,print("ok1");
    if((c-x1)+x2<=W and c-x1>=0)ans=min(ans,1.0*(c-x1));
    if(y1-(y2-b)>=0 and y2-b>=0)ans=min(ans,1.0*(y2-b));//print(ans);
    /// left down
    a=0,b=0,c=w,d=h;
    //print(a,b,c,d);
    if(alabu(a,b,c,d))ans=min(ans,k);//,print("ok2");
    if((d-y1)+y2<=H and d-y1>=0)ans=min(ans,1.0*(d-y1));
    if((c-x1)+x2<=W and c-x1>=0)ans=min(ans,1.0*(c-x1));//print(ans);
    /// right up
    a=W-w,b=(H-h),c=W,d=H;//print(a,b,c,d);
    if(alabu(a,b,c,d))ans=min(ans,k);//,print("ok3");
    if(y1-(y2-b)>=0 and y2-b>=0)ans=min(ans,1.0*(y2-b));
    if(x1-(x2-a)>=0 and x2-a>=0)ans=min(ans,1.0*(x2-a));//print(ans);

    /// right down
    a=W-w,b=0,c=W,d=h;//print(a,b,c,d);
    if(alabu(a,b,c,d))ans=min(ans,k);//,print("ok4");
    if((d-y1)+y2<=H and d-y1>=0)ans=min(ans,1.0*(d-y1));
    if(x1-(x2-a)>=0 and x2-a>=0)ans=min(ans,1.0*(x2-a));
    if(ans==INF)print(-1);
    else
