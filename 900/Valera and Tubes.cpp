...
void solve()
{
    ...
    rep1(i,1,n)
    {
        if(i&1)
        {
            for(v99 j=1;j<=m;j++)a.push(i),a.push(j);
        }
        else
        {
            for(v99 j=m;j>0;j--)a.push(i),a.push(j);
        }
    }
    while(k>1 and a.size()>3)
    {
        cout<<2<<whp;
        for(v99 i=0;i<4;i++)cout<<a.front()<<whp,a.pop();
        cout<<ses;
        k--;
    }
    cout<<a.size()/2<<whp;
    while(a.size())cout<<a.front()<<whp,a.pop();
    cout<<ses;
}
