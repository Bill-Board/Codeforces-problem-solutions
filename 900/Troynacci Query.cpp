...
v99 a,b,f[mxi],p[mxi],vp[mxi],ek[mxi],mod=1e9+7;


void solve()
{
    /// code is here->
    v99 n,q;read(n,q,f[1],f[2],a,b);
    rep1(i,1,n)read(vp[i]);
    for(v99 i=3;i<=n+100;i++)f[i]=(a*f[i-2] + b*f[i-1])%mod;
    while(q--)
    {
        v99 l,r;read(l,r);
        if(l!=r)
        {
            p[l]=(p[l]+f[1])%mod;
            p[l+1]=((p[l+1]+f[2]-(b*f[1])%mod)+mod)%mod;
            p[r+1]=((p[r+1] -f[r-l+2])+mod)%mod;
            p[r+2]=((p[r+2] -(a*f[r-l+1])%mod)+mod)%mod;
        }
        else ek[l]=(ek[l]+f[1])%mod;
    }
    rep1(i,2,n)p[i]=(p[i]+(a*p[i-2])%mod +(b*p[i-1])%mod )%mod;
    rep1(i,1,n)cout<<(p[i]+vp[i]+ek[i])%mod<<whp ses[i==n];
