...
string alabu(string s)
{
    ...
    string p=to_string(sum),ans;
    reverse(sob(s));reverse(sob(p));
    while(s.size()>p.size())p+='0';
    v99 c=0;
    for(v99 i=0;i<s.size();i++)
    {
        v99 niche=c+(p[i]-'0'),upore=s[i]-'0';
        if(niche>upore)
        {
            c=1;
            upore+=10;
...
void solve()
{
    ...
    string mx_id="";
    for(v99 i=1;i<=(n-2);i++)mx_id+='9';
    mx_id+="50";
    string mn_id="1";
    for(v99 i=2;i<n;i++)mn_id+='0';
    mn_id+="9";
    print(mx_id);
    print(mn_id);
    print(alabu(mx_id));
