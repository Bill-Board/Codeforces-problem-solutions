...
void solve()
{
    ...
    rep1(i,1,n-1)
    {
        //print(dist[ per[i] ] , dist[per[i+1]]);
        if(dist[ per[i] ]>=dist[per[i+1]])return print(-1);

    }
    rep1(i,1,n)
    {
        if(i==rt and ans[i]<0)return print(-1);
        if(i!=rt and ans[i]<=0)return print(-1);
    }
    rep1(i,1,n)cout<<ans[i]<<whp;cout<<ses;
    //rep1(i,1,n)cout<<dist[i]<<whp;cout<<ses;
}
