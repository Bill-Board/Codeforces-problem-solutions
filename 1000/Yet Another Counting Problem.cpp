...
v99 accha(v99 l,v99 r,v99 l_m,v99 m)
{
    v99 cr_h=0;
    v99 vag=(l+l_m-1)/l_m;
    v99 n_l=vag*l_m;
    if(l%l_m<m and l%l_m)
    {
        v99 kot=m-(l%l_m);
        cr_h+=min(r-l+1,kot);
    }
    vag=(r/l_m);v99 n_r=vag*l_m;
    //print(n_l,n_r);
    if(n_l<=r)cr_h+=min(m,r-n_r+1);
    v99 ans=max(0LL,n_r-n_l)/l_m;
    return (ans*m)+cr_h;
}
...
{
    /// code is here->
    v99 a,b,q;read(a,b,q);
    v99 l_m=lcm(a,b),m=max(a,b);
    while(q--)
    {
        v99 l,r;read(l,r);
        cout<<r-l+1-accha(l,r,l_m,m)<<whp;
    }
    cout<<ses;
}
