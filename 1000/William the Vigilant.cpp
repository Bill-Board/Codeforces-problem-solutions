...
{
    /// code is here->
    v99 n,q,kot=0;read(n,q);
    string s;read(s);s='1'+s+"235";
    vll cnt(n+2);
    for(v99 i=1;i<=n;)
    {
        if(s[i]=='a' and s[i+1]=='b' and s[i+2]=='c')
        {
            cnt[i]=1;cnt[i+1]=2;cnt[i+2]=3;
            kot++;
            i+=3;
        }
        else i++;
    ...
    while(q--)
    {
        v99 pos;char c;read(pos,c);
        if(s[pos]==c)print(kot);
        else
        {
            if(c=='a')
            {
                if(cnt[pos]==2)
                    cnt[pos-1]=cnt[pos]=cnt[pos+1]=0,kot--;
                else if(cnt[pos]==3)
                {
                    cnt[pos-1]=cnt[pos-2]=cnt[pos]=0;
                    kot--;
                    string p;
                    if(pos+1<=n)p+=s[pos+1];
                    if(pos+2<=n)p+=s[pos+2];
                    if(p=="bc")
                    {
                        kot++;
                        cnt[pos]=1;cnt[pos+1]=2;cnt[pos+2]=3;
                    }
                }
                else
                {
            ...
            if(c=='b')
            {
                ...
                {
                    string p;
                    if(pos-1>0)p+=s[pos-1];
                    if(pos+1<=n)p+=s[pos+1];
                    if(p=="ac")
                    {
                        kot++;
                        cnt[pos-1]=1;cnt[pos]=2;cnt[pos+1]=3;
                    }
                }
            }
            if(c=='c')
            {
                ...
                else if(cnt[pos]==1)
                {
                    ...
                    string p;
                    if(pos-1>0)p+=s[pos-1];
                    if(pos-2>0)p+=s[pos-2];
                    if(p=="ba")
                    {
                        kot++;
                        cnt[pos-2]=1;cnt[pos-1]=2;cnt[pos]=3;
                    }
                }
                else
                {
