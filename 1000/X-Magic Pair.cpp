...
v99 age_sob;
vll mult_sob;
void alabu(v99 a,v99 b)
{
    if(a>b)swap(a,b);
    s.insert({a,b});
}

bool src(v99 a,v99 b)
{
    if(a>b)swap(a,b);
    return s.find({a,b})==s.end();
}

void dfs(v99 a,v99 b)
{
    if(a>b)swap(a,b);
    v99 dif=abs(a-b);
    alabu(a,b);
    mpp[dif]++;
    if(b%a==0){mult_sob.pb(a);return;}
    if(b-a==1){age_sob=max(age_sob,b);return;}

    if(src(a,dif))dfs(a,dif);
    if(src(b,dif))dfs(b,dif);
}
void init()
{
    s.clear();
...
void solve()
{
    /// code is here->
    init();
    v99 a,b,x;read(a,b,x);
    if(a>b)swap(a,b);
    mpp[a]++;mpp[b]++;
    dfs(a,b);
    bool ache=false;
    if(x<=age_sob or mpp[x])ache=true;
    for(auto i:mult_sob)
    {
        if(x<=a and x%i==0)ache=true;
    }
    if(ache)print("YES");
    else print("NO");
}
