...
void solve()
{
    ...
    sort(sob(pos),greater<>());
    sort(sob(neg),greater<>());
    v99 cr_h=0;
    if(pos.size()>k)
    {
       v99 val=0;
       while(pos.size()%k!=0)val=pos.back(),pos.pop_back();
       cr_h+=2*val;
    }
    if(neg.size()>k)
    {
       v99 val=0;
       while(neg.size()%k!=0)val=neg.back(),neg.pop_back();
       cr_h+=2*val;
    }
    while(pos.size()>k or neg.size()>k)
    {
        if(pos.size()>k)
        {
            v99 sum=0;
            for(v99 i=0;i<k and pos.size()>k;i++)
            {
                sum=pos.back();
                pos.pop_back();
            }
        ...
        {
            v99 sum=0;
            for(v99 i=0;i<k and neg.size()>k;i++)
            {
                sum=neg.back();
                neg.pop_back();
            }
    ...
    v99 ek=0,dui=0;
    if(pos.size())ek=pos.front();
    if(neg.size())dui=neg.front();
    v99 ans1=cr_h+(2*ek)+dui,ans2=cr_h+ek+(2*dui);
    print(min(ans1,ans2));
}
