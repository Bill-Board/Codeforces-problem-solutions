...
///-------------------------------------------------------------------------------------------------------
v99 lf,fixlf,visi[mxi],v[mxi];
vll tom[mxi];
vector<pair< pii,pii > >vp;
void dfs(v99 ind)
{
    visi[ind]=1;
    if(tom[ind].size()==1)lf=ind,fixlf=ind;
    for(auto i:tom[ind])if(visi[i]==0)dfs(i);
}

void dfs2(v99 ind,v99 p)
{
    v[ind]=1;
    for(auto i:tom[ind])if(v[i]==0)dfs2(i,ind);
    if(tom[ind].size()<=2)return;
    v99 del=0;
    for(v99 i=0;i<tom[ind].size();i++)
    {
        if(tom[ind][i]==p or tom[ind][i]==fixlf)continue;
        vp.pb({{ind,tom[ind][i]}, {lf, tom[ind][i]}});
        lf=tom[ind][i];
        del++;
        if(tom[ind].size()-del<3)break;
    }
}
...
void solve()
{
    ...
    v99 n;read(n);
    vp.clear();
    for(v99 i=0;i<=(n+2);i++)tom[i].clear(),v[i]=visi[i]=0LL;
    for(v99 i=0;i<n-1;i++)
    {
        v99 u,v;read(u,v);
        tom[u].pb(v);
    ...
    rep1(i,1,n)if(visi[i]==0)dfs(i);
    rep1(i,1,n)if(v[i]==0)dfs2(i,0LL);
    print(vp.size());
    for(auto i:vp)print(i.ff.ff,i.ff.ss,i.ss.ff,i.ss.ss);
}
