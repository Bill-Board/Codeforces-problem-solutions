...
v99 n,m,l[5005],r[5005],p[5005],q[5005];
v99 ok(v99 ind)
{
    rep1(i,0,n)p[i]=q[i]=0;
    rep1(i,0,m)
    {
        if(i==ind)continue;
        p[l[i]]++;p[r[i]+1]--;
    }
    rep1(i,1,n)p[i]+=p[i-1];
    rep1(i,1,n)q[i]=q[i-1]+(p[i]==1);
    v99 cr_h=-LLONG_MAX;
    rep0(i,0,m)
    {
        if(i==ind)continue;
        cr_h=max(cr_h,q[l[i]-1]-q[r[i]]);
    }
    rep1(i,1,n)if(p[i]>0)cr_h++;
    return cr_h;
}
...
void solve()
{
    /// code is here->
    read(n,m);v99 cr_h=0;
    rep0(i,0,m)read(l[i],r[i]);
    rep0(i,0,m)cr_h=max(cr_h,ok(i));
    cout<<cr_h<<ses;
}
