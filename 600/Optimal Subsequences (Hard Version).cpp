...
void solve()
{
    ...
    vll a(n);
    vector<pii>vp;
    rep0(i,0,n)cin>>a[i],vp.pb({a[i],i});
    sort(sob(vp),sort2val);
    read(m);
    vector<pair<pii,v99> > v(m);
    rep0(i,0,m)read(v[i].ff.ff,v[i].ff.ss),v[i].ss=i;
    sort(sob(v));
    v99 len=0;
    ott s;
    vll ans(m);
    rep0(i,0,m)
    {
        while(len<v[i].ff.ff)
            s.insert(vp[len++].ss);
        ans[v[i].ss]=a[*s.find_by_order(v[i].ff.ss-1)];
    }
    rep0(i,0,m)print(ans[i]);
}
