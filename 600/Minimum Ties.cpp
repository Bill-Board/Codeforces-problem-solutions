...
void solve()
{
    ...
    for(v99 dif=1;dif<n;dif++)
    {
        set<pii>pp;
        for(v99 i=1,j=i+dif;i<=n;i++,j++)
        {
            if(j>n)j=j%n;
            if(p.find({i,j})==p.end() and p.find({j,i})==p.end())p.insert({i,j}),pp.insert({i,j});

        }
        if(pp.size()!=n){ for(pii i:pp)p.erase(p.find(i));break;}
    }
    //for(pii i:p)print(i.ff,i.ss);
    rep1(i,1,n)
    {
        rep1(j,i+1,n)
        {
            if(p.find({i,j})!=p.end())cout<<1<<whp;
            else if(p.find({j,i})!=p.end())cout<<-1<<whp;
            else cout<<0<<whp;
        }
    }
    cout<<ses;
