...
{
    /// code is here->
    v99 n;read(n);vll a,b;
    priority_queue <v99, vector<v99>, greater<v99> > qw,er;
    rep0(i,0,n)
    {
        v99 x;read(x);
        if(i&1)a.pb(x);
    ...
    v99 sum1=0,sum2=0,cr_h=LLONG_MAX,ek=0,dui=0;
    for(v99 i=0,j=n;i<max(a.size(),b.size());i++,--j)
    {
        if(i<b.size())qw.push(b[i]),sum1+=b[i],ek=(sum1-qw.top())+(j*qw.top());
        if(i!=0)cr_h=min(cr_h,ek+dui);
        if(i<a.size())er.push(a[i]),sum2+=a[i],dui=(sum2-er.top())+(j*er.top());
        cr_h=min(cr_h,ek+dui);
    }
    print(cr_h);
}
