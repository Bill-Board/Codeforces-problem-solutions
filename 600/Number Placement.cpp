...
void SieveOfEratosthenes(v99 n)
{
	...
	for(v99 i=3;i<=n;i+=2)if(!prime[i])prm.pb(i+prm.back());
}
...
v99 cnt,n,visi[802][802],ans[4*mxi];
string s[803];

void dfs(v99 indx ,v99 indy)
{
    visi[indx][indy]=1;cnt++;
    rep0(i,0,4)
    {
        v99 x=fx[i]+indx,y=fy[i]+indy;
        if(0<=x and x<n and 0<=y and y<n and s[x][y]=='.' and visi[x][y]==0)
            dfs(x,y);
    }
}
...
{
    /// code is here->
    read(n);SieveOfEratosthenes(9604001LL);
    rep0(i,0,n)read(s[i]);
    v99 cr_h=0;
    rep0(i,0,n)
    {
