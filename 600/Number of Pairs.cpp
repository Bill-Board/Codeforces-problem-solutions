...
void solve()
{
    ...
    rep0(i,0,n)
    {
        v99 pos=i+1;
        if(a[i]>=r)break;
        v99 up_val=r-a[i],low_val=l-a[i];
        if(a[i]+a[n-1]<l)continue;
        v99 pos_u=upper_bound(sob(a),up_val)-a.begin();
        v99 pos_l=lower_bound(sob(a),low_val)-a.begin();
        //print(pos_l);
        if(a[pos_l]+a[i]>=l)pos_l++;
        //print(low_val,pos_l,pos_u);
        pos_l=max(pos_l,pos+1);
        cr_h+=max(0LL,pos_u-pos_l+1);
        //print(max(0LL,pos_u-pos_l+1));
    }
    print(cr_h);
}
