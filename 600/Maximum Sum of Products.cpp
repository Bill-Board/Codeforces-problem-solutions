...
///-------------------------------------------------------------------------------------------------------
v99 a[5003],b[5003],suf[5003],pre[5003],pro[5005][5005];
...
void solve()
{
    ...
    rep1(i,1,n)read(b[i]);
    v99 kot=0;
    rep1(i,1,n)kot+=(a[i]*b[i]),pre[i]=kot;
    kot=0;
    for(v99 i=n;i>0;i--)kot+=(a[i]*b[i]),suf[i]=kot;
    for(v99 i=1;i<=n;i++)pro[i][i]=a[i]*b[i];
    for(v99 len=1;len<n;len++)
    {
        for(v99 l=1;l<=n;l++)
        {
            v99 r=l+len;
            if(r>n or l==r)continue;
            pro[l][r]=a[l]*b[r] + a[r]*b[l] +pro[l+1][r-1];
        }
    }
    v99 cr_h=0;
    for(v99 i=1;i<=n;i++)
    {
        for(v99 j=i;j<=n;j++)cr_h=max(cr_h,pre[i-1]+suf[j+1]+pro[i][j]);
    }
    print(cr_h);
}
