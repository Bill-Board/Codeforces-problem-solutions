...
void solve()
{
    ...
    sort(sob(vp));
    for(v99 i=0;i<m;i++)ans[vp[i].ss][i+1]=vp[i].ff,s[vp[i].ss].erase(s[vp[i].ss].find(vp[i].ff));
    for(v99 i=1;i<=n;i++)
    {
        v99 ind=1;
        for(auto j=s[i].begin();j!=s[i].end();ind++)
        {
            if(ans[i][ind])continue;
            ans[i][ind]= *j;
            j++;
        }
    }
    for(v99 i=1;i<=n;i++)
    {
        rep1(j,1,m)cout<<ans[i][j]<<whp;cout<<ses;
    }
}
