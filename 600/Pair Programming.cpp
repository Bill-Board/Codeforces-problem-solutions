...
//    sort(sob(b),greater<>());
    ...
    while(a.size() or b.size())
    {
        if(a.empty())
        {
            if(b.back()==0)k++,ans.pb(0LL);
            else if(b.back()>k)return print(-1);
            else ans.pb(b.back());
            b.pop_back();
        }
        else if(b.empty())
        {
            if(a.back()==0)k++,ans.pb(0LL);
            else if(a.back()>k)return print(-1);
            else ans.pb(a.back());
            a.pop_back();
        }
        else
