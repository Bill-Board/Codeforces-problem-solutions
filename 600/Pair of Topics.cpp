...
void del(v99 val)
{
    auto pos= lower_bound(sob(dif),val)-dif.begin();
    dif.erase(dif.begin()+pos);
}
v99 koita(v99 val)
{
    v99 pos=lower_bound(sob(dif),val)-dif.begin();
    return pos;
}
...
{
    /// code is here->
    v99 n,cr_h=0;read(n); vll a(n),b(n);
    read(a);read(b);
    rep0(i,0,n)dif.pb(b[i]-a[i]);
    sort(sob(dif));
    rep0(i,0,n)
    {
        v99 val=(a[i]-b[i]);
