...
{
    /// code is here->
    v99 n;read(n);vll a(2*n),b;map<v99,v99>mpp;
    rep0(i,0,2*n)cin>>a[i],mpp[a[i]]++;
    for(auto i:mpp)
    {
        if(i.ss!=2 or i.ff&1){print("NO");return;}
        b.pb(i.ff/2);
    }
    sort(sob(b),greater<>());
    v99 sum=0;//print(b);
    for(v99 i=0,j=n;i<n;i++,j--)
    {
        v99 kot=b[i]-sum;
        if(kot<=0){print("NO");return;}
        if(kot%j!=0){print("NO");return;}
        sum+=(kot/j);//print(kot,sum);
    }
    print("YES");
