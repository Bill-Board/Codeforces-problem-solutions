...
{
    /// code is here->
    v99 n,u,v;read(n,u,v);vll a(n);
    read(a);
    rep0(i,0,n-1)if(abs(a[i]-a[i+1])>1){print(0);return;}
    rep0(i,0,n-1)if(abs(a[i]-a[i+1])==1){print(min(v,u));return;}
    rep0(i,0,n-1)if(abs(a[i]-a[i+1])==0){print(v+min(v,u));return;}
}
