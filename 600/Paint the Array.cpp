...
void solve()
{
    ...
    vll a(n);read(a);
    v99 g=0;
    for(v99 i=0;i<n;i+=2)g=gcd(g,a[i]);
    bool ok=true;
    for(v99 i=1;i<n;i+=2)ok&=(a[i]%g!=0);
    if(ok)return print(g);
    g=0;
    for(v99 i=1;i<n;i+=2)g=gcd(g,a[i]);
    for(v99 i=0;i<n;i+=2)if(a[i]%g==0)return print(0);
    print(g);
}
