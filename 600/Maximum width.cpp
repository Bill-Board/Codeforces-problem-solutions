...
void solve()
{
    ...
    v99 n,m;read(n,m);
    string s,t;read(s,t);
    vll pre({0LL}),suf({0LL});//print(pre);print(suf);
    ///prefix r jnno
    for(v99 i=0,j=0;i<n and j<m;i++)
    {
        while(s[i]==t[j] and j<m and i<n)pre.pb(i+1),i++,j++;
    }
    for(v99 i=n-1,j=m-1;~i and ~j;i--)
    {
        while(s[i]==t[j] and j>=0 and i>=0)suf.pb(i+1),i--,j--;
    }
    suf.pb(0LL);pre.pb(0LL);reverse(sob(suf));
    //print(suf);print(pre);
    v99 cr_h=0;
    for(v99 i=1;i<m;i++)cr_h=max(cr_h,abs(pre[i]-suf[i+1]));
    for(v99 i=2;i<=m;i++)cr_h=max(cr_h,abs(pre[i]-suf[i-1]));
    print(cr_h);
}
