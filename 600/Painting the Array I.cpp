...
{
    /// code is here->
    v99 n,m=0,mm=0;read(n);
    vll p(n);read(p);
    v99 a[n+2],b[n+2];a[0]=b[0]=-1;
    rep0(i,0,n)
    {
        v99 x=p[i];
        if(a[m]!=x and b[mm]!=x )
        {
            if(i+1<n and p[i+1]==a[m])a[++m]=x;
            else b[++mm]=x;
        }
        else if(a[m]!=x)a[++m]=x;
        else if(b[mm]!=x)b[++mm]=x;
    }
    //rep1(i,1,m)cout<<a[i]<<whp ses[i==m];
    //rep1(i,1,mm)cout<<b[i]<<whp ses[i==mm];
    print(mm+m);
}
