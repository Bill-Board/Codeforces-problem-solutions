...
void solve()
{
    /// code is here->
    v99 n,m;read(n,m);
    v99 h=abs(1-n)+abs(1-m);
    vll r(n,1),c(m,1),p{h},ans{h};
    rep0(i,0,(n+1)/2)r[i]=-1;
    rep0(i,0,(m+1)/2)c[i]=-1;
    if(n%2==0)r[n/2]=0;
    if(m%2==0)c[m/2]=0;
    ...
    for(v99 i=1;i<n;i++)
    {
        ...
        p=a.back();
        vll q;
        for(auto j:p)q.pb(j+r[i]),ans.pb(q.back());
        a.pb(q);
    }
    sort(sob(ans));
    print(ans);
