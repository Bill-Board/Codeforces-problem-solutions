...
v99 h,m,ulta[]={0,1,5,-1,-1,2,-1,-1,8,-1};
...
v99 valid(v99 n)
{
    ...
    for(v99 i=1;~i;i--)
    {
        if( ulta[s[i]-'0']==-1)return LLONG_MAX;
        ans+=char( ulta[s[i]-'0']+'0');
    }
    return stoll(ans);
}
...
void solve()
{
    ...
    read(h,m);
    string s;read(s);
    v99 H=(s[0]-'0')*10 +s[1]-'0';
    v99 M=(s[3]-'0')*10 +s[4]-'0';
    while(true)
    {
        if(M==m)M=0,H++;
        if(H==h)H=0;
        if(valid(M)<h and valid(H)<m)
        {
            printf("%.2lld:%.2lld\n",H,M);
            return;
        }
