...
void dfs(v99 ind)
{
    ...
    for(auto i:tom[ind])
    {
        ...
        {
            dfs(i);
            a[ind][0]+=max(a[i][0]+abs(vp[ind].ff-vp[i].ff),  a[i][1]+abs(vp[ind].ff-vp[i].ss));
            a[ind][1]+=max(a[i][0]+abs(vp[ind].ss-vp[i].ff),  a[i][1]+abs(vp[ind].ss-vp[i].ss));
        }
    }
...
void init(v99 n)
{
    ...
    {
        tom[i].clear();
        a[i][0]=a[i][1]=visi[i]=0LL;
    }
}
void solve()
{
    /// code is here->
    v99 n;read(n);vp.resize(n+2);
    init(n+3);
    rep1(i,1,n)read(vp[i].ff,vp[i].ss);
    rep0(i,0,n-1)
    {
        v99 u,v;
        read(u,v);
