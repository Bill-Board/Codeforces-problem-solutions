...
void solve()
{
    /// code is here->
    v99 n,cr_h=0;read(n);
    vll a(n),b(n),pre(n+2,0);
    read(a);read(b);
    sort(sob(a));sort(sob(b));
    v99 k=n- n/4,pos1=n/4 ,pos2=pos1,sum1=0,sum2=0;
    for(v99 i=pos1;i<n;i++)sum1+=a[i],sum2+=b[i];
    reverse(sob(b));
    rep1(i,1,n)pre[i]=pre[i-1]+b[i-1];
    pos2--;
    //if(n%4==0)pos1--;
    //print(sum1,sum2,pos1,pos2);
    while(sum1<sum2)
    {
        sum1+=100;a.pb(100LL);
        if(a.size()%4==0 and pos1>=0)sum1-=a[pos1++];
        k=(a.size()- a.size()/4);
        sum2=pre[min(n,k)];
        cr_h++;
        //print(sum1,sum2);
