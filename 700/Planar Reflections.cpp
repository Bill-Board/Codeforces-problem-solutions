...
{
    /// code is here->
    v99 n,k,mod=1000000007LL;
    read(n,k);
    v99 cr_h=1+n;
    if(k==1)return print(1);
    if(k==2)return print(1+n);
    if(n==1)return print(2);
    vll a;
    for(v99 i=n-1;i>0;i--)a.pb(i);
    for(auto j:a)cr_h=(cr_h+j)%mod;
    for(v99 i=1;i<=k-3;i++)
    {
        if(a.empty())break;
        if(i&1){
            if(a.size()>1){for(v99 j=1;j<a.size();j++)a[j]=((a[j] %mod )+(a[j-1]%mod))%mod;}
        }
        else
        {
            if(a.size()>1){for(v99 j=a.size()-2;j>=0;j--)a[j]=((a[j] %mod )+(a[j+1]%mod))%mod;}
        }
        for(auto j:a)cr_h=(cr_h+j)%mod;
    }
    print(cr_h);
