...
void change()
{
    ...
    else if(xa==xb and ya>yb)swap(ya,yb);
}
void ok_print(v99 x,v99 y)
{
    if((x==xa and y==ya) or (x==xb and y==yb))return;
    cout<<x<<whp<<y<<whp;
}
...
void solve()
{
    /// code is here->
    read(xa,ya,xb,yb);
    v99 dif_a=abs(xa-xb),dif_b=abs(ya-yb);
    if(dif_a!=dif_b and dif_a>0 and dif_b>0)return print(-1);
    change();
    v99 d=max(dif_a,dif_b);
    v99 x=min(xa,xb);
    v99 y=min(ya,yb);
    ok_print(x,y);
    ok_print(x,y+d);
