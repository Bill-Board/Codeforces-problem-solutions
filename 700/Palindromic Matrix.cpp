...
{
    /// code is here->
    v99 n;read(n);v99 a[n*n +2],b[n+2][n+2];
    map<v99,v99>mpp;
    rep0(i,0,n*n)
    {
        read(a[i]);
    ...
    for(auto i:mpp)q.push({i.ss,i.ff});
    v99 loop=n/2;
    for(v99 i=1;i<=loop;i++)
    {
        for(v99 j=1;j<=loop;j++)
        {
            set<pii>s;
            s.insert({i,j});
            s.insert({i,n-j+1});
            ...
            v99 ke=q.top().ss,koita=q.top().ff;q.pop();
            if(koita<s.size()){print("NO");return;}
            for(pii r:s)b[r.ff][r.ss]=ke;
            koita-=s.size();
            if(koita)q.push({koita,ke});
        }
    }
    ...
    if(n&1)
    {
        ...
        if(q.size()!=1){print("NO");return;}
        v99 ke=q.top().ss,koita=q.top().ff;
        if(koita!=1){print("NO");return;}
        b[loop][loop]=ke;
    }
    print("YES");
    for(v99 i=1;i<=n;i++)
    {
        for(v99 j=1;j<=n;j++)cout<<b[i][j]<<whp;
        cout<<ses;
    }
