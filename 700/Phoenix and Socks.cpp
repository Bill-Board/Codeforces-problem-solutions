...
void ok(v99 l[],v99 r[])
{
    rep1(i,1,n)cout<<l[i]<<whp ses[i==n];
    rep1(i,1,n)cout<<r[i]<<whp ses[i==n];
}

v99 alabu(v99 l[],v99 r[],v99 L,v99 R)
{
    v99 cr_h=0;
    for(v99 i=1;i<=n;i++)
    {
    ...
    for(v99 i=1;L!=R and i<=n;i++)
    {
        ...
        L+=m;R-=m;cr_h+=m;
        //print(i,L,R);
        //ok(l,r);print(ses,ses);
    }
    v99 sum=0;
    for(v99 i=1;i<=n;i++)sum+=min(l[i],r[i]);
    return cr_h+R-sum;
}
...
void solve()
{
    /// code is here->
    v99 L,R;read(n,L,R);
    v99 l[n+4],r[n+4],a[n+2];
    for(v99 i=1;i<=n+1;i++)l[i]=r[i]=0;
    for(v99 i=1;i<=n;i++)
    {
        read(a[i]);
        if(i<=L)l[a[i]]++;
    ...
    if(L<=R)print(alabu(l,r,L,R));
    else print(alabu(r,l,R,L));
