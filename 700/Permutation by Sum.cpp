...
void alabu()
{
    v99 sum=0,n=250*601;//valid[0LL]=-1;
    for(v99 i=1;sum<=n;i++)sum+=i,valid[sum]=i;
    for(v99 i=1;i<=n;i++)if(valid[i]==0)valid[i]=valid[i-1];
...
{
    /// code is here->
    v99 n,l,r,s;read(n,l,r,s);
    v99 k=r-l+1;
    vll a;
    set<v99>ss;
    for(v99 i=n;i>0 and k;i--)
    {
        v99 ekn=s-i;
        if(ekn<0)continue;
        //print(valid[ekn]);
    ...
    if(a.size()!=(r-l+1) or k>0 or s>0)return print(-1);
    for(v99 i=1,j=1;j<l;i++)
    {
        if(ss.find(i)==ss.end())
        {
            cout<<i<<whp;
