...
v99 alabu()
{
    v99 one=0,two=0,baki_1=5,baki_2=5;
    rep0(i,0,s.size())
    {
        if(i%2==0)one+=s[i]=='1',--baki_1;
        else two+=s[i]=='1',--baki_2;
        if(one+baki_1<two or two+baki_2<one)return i+1;
    }
    return 10LL;
}
...
void solve()
{
    ...
    read(s);
    vll a;
    rep0(i,0,s.size())if(s[i]=='?')a.pb(i);
    v99 cr_h=LLONG_MAX;
    for(v99 i=0;i<(1<<a.size());i++)
    {
        rep0(j,0,a.size())
        {
            if(i&(1<<j))s[a[j]]='1';
