...
void init()
{
    ...
    //rep1(i,1,10){for(auto j:tom[i])cout<<j<<whp;cout<<ses;}
}
...
void solve()
{
    /// code is here->
    v99 n;read(n);
    v99 kot=tom[n+1].back();
    if(n&1)return print(kot);
    v99 ok=tom[n/2 +1].back();
    print((kot+mod-ok)%mod);
}
