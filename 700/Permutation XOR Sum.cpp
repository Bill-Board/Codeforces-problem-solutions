...
void solve()
{
    ...
    {
        v99 ulta=alabu(i);
        if(ulta==0){i--;continue;}
        v99 ans=(abs(i-ulta)+1)*max(okay(i),okay(ulta));
        //print(i,ulta,okay(i),okay(ulta));
        i=min(i-1,ulta-1);
        cr_h+=ans;
    }
    print(cr_h);
...
vll a,ans;
    ...
    {
        v99 sum=0;
        rep1(i,1,n)sum+=(a[i-1]^i);
        if(cr_h<sum)cr_h=sum,ans=a;
    }while(next_permutation(sob(a)));
    print(ans);
    print("ans ",cr_h,ses,ses);
