...
bool ek(vector<pii>vp)
{
    deque<pii>dq;
    set<pii>q;
    for(v99 i=0;i<m;i++)
    {
        q.insert({vp.back().ff,i});
        ans[vp.back().ss]=i+1;
        vp.pop_back();
    }
    for(auto i:vp)dq.push_back({i.ff,i.ss});
    while(dq.size())
    {
        v99 f=q.begin()->ff,f_p=q.begin()->ss;
        q.erase(q.begin());
        q.insert({f+dq.front().ff,f_p});
        ans[dq.front().ss]=f_p+1;
        dq.pop_front();
    }
    v99 f=q.begin()->ff,l=prev(q.end())->ff;
    return (l-f)<=x;
}
bool dui(vector<pii>vp)
{
    deque<pii>dq;
    set<pii>q;
    reverse(sob(vp));
...
void solve()
{
    ...
    read(n,m,x);
    vector<pii>vp(n);
    rep0(i,0,n)read(vp[i].ff),vp[i].ss=i;
    sort(sob(vp));
    ans.resize(n);
    if(ek(vp) or dui(vp))
    {
