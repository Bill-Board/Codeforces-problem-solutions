...
///-------------------------------------------------------------------------------------------------------
string alabu(string s)
{
    string p;
    for(auto i:s)
    {
        if(p.empty())p.pb(i);
        else if(p.back()=='(' and i==')')p.pop_back();
        else p.pb(i);
    }
    return p;
}
v99 koita(string s,char c)
{
    v99 cnt=0;
    for(auto i:s)cnt+=i==c;
    return cnt;
}
void solve()
{
    ...
    string s;read(s);
    string p=alabu(s);
    v99 open=koita(p,'('),close=koita(p,')');
    v99 m=min(open,close);
    v99 ans=abs(open-close)*a;/// barti gula delete kore disi
    open=m;close=m;/// ekohn duitai ii soman
    if(2*a<b)ans+=(open+close)*a;
    else ans+=open*b;
    print(ans);
}
