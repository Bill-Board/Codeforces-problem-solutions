...
///-------------------------------------------------------------------------------------------------------
void print(deque<pii>q)
{
    while(q.size())print(q.front().ff,q.front().ss),q.pop_front();cout<<ses<<ses;
}
...
void solve()
{
    ...
    while(dq.size())
    {
        v99 dis=dq.front().ff,kinbo=dq.front().ss;
        while(dis>koita and dq.size())
        {
            v99 ok=dis-koita;
            v99 mn=min(ok,dq.back().ss);
            koita+=mn;
            if(mn==dq.back().ss)dq.pop_back();
            else dq.back().ss-=mn;
            cr_h+=(mn*2);
        }
        if(dq.size())koita+=dq.front().ss,cr_h+=dq.front().ss,dq.pop_front();
    }
    print(cr_h);
}
