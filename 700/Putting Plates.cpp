...
///-------------------------------------------------------------------------------------------------------
v99 n,m,a[22][22],k=0,visi[22][22];

void dfs(v99 indx,v99 indy)
{
    if(visi[indx][indy])return;
    if(indx!=1 and indx!=n and indy!=1 and indy!=m)return;
    if(indx<1 or indx>n or indy<1 or indy>m)return;
    visi[indx][indy]=1;
    //print(indx,indy);
    bool ok=false;
    for(v99 j=0;j<8;j++)
    {
        v99 xx=indx+ox8[j],yy=indy+oy8[j];
        if(a[xx][yy]==1)
        {
            ok=true;
            break;
...
void clr()
{
    rep1(i,0,n+1)rep1(j,0,m+1)visi[i][j]=a[i][j]=0;
}
...
void solve()
{
    ...
    if(cr_h<k)
    {
        rep1(i,1,n)rep1(j,1,m)ans[i][j]=a[i][j];
        cr_h=k;
    }
    //rep1(i,1,n){rep1(j,1,m)cout<<a[i][j];cout<<ses;}print(k);
    clr();
    a[1][2]=1;
    k=0;
    dfs(1,3);
    ...
    //rep1(i,1,n){rep1(j,1,m)cout<<a[i][j];cout<<ses;}print(k);
    rep1(i,1,n){rep1(j,1,m)cout<<ans[i][j];cout<<ses;}
