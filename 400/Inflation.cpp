...
///-------------------------------------------------------------------------------------------------------
bool alabu(vll a,v99 mid,v99 k)
{
    v99 sum=0;a[0]+=mid;
    for(v99 i=0;i<a.size()-1;i++)
    {
        v99 niche=(a[i]+sum)*k;
        v99 upore=a[i+1]*100LL;
        if(upore>niche)return false;
        //print(upore,niche);
        sum+=a[i];
    }
    return true;
...
{
    /// code is here->
    v99 n,k;read(n,k);vll a(n);read(a);
    v99 sum=0,cr_h=LLONG_MAX, l=0LL,r=1e12;
    while(l<r)
    {
        v99 mid=(l+r)/2;//print(l,r,mid);
        if(alabu(a,mid,k))r=mid,cr_h=mid;
        else l=mid+1;
    }
    print(cr_h);
}
