...
///-------------------------------------------------------------------------------------------------------
v99 a=0,b=0,c=0;
v99 na,nb,nc,pa,pb,pc;
bool is_possible(v99 val,v99 r)
{

    v99 lagbe=val*a;
    lagbe-=na;
    if(lagbe>0 and pa*lagbe>r)return false;
    if(lagbe>0)r-=pa*lagbe;


    lagbe=val*b;
    lagbe-=nb;
    if(lagbe>0 and pb*lagbe>r)return false;
    if(lagbe>0)r-=pb*lagbe;

    lagbe=val*c;
    lagbe-=nc;
    if(lagbe>0 and pc*lagbe>r)return false;
    if(lagbe>0)r-=pc*lagbe;
    return r>=0;
}
