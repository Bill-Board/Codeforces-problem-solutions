...
v99 x,y,a,b;
bool is_possible(v99 n)/// n ta set korte parbo kina
{
    v99 l=0,r=n;
    while(l<=r)
    {
        v99 mid=(l+r)/2,baki=n-mid;
        v99 kot=mid*a + baki*b,kot2=baki*a+mid*b;
        if(kot<=x and kot2<=y)return true;
        if(kot>x and kot2>y)return false;
        if(kot>x)r=mid-1;
        else l=mid+1;
    }
    return false;
...
void solve()
{
    ...
    read(x,y,a,b);
    if(a<b)swap(a,b);
    if(a==b)return print((v99)(min(x,y)/a ));
    v99 l=0,r=max(x,y);
    while(r-l>1)
    {
        v99 mid=(l+r)/2;
