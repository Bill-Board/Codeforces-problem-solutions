...
void solve()
{
    ...
    for(v99 i=3;i<=n;i++)
    {
        v99 kot=ask(1LL,2LL,i);
        a.pb({kot,i});
    }

    sort(sob(a),greater<>());
    v99 ek=a[0].ss,dui=a[1].ss;

    for(v99 i=1;i<=n;i++)
    {
        if(i==ek or i==dui)continue;
        v99 kot=ask(ek,dui,i);
        b.pb({kot,i});
        s.insert(kot);
    }
    sort(sob(b),greater<>());
    ...
    v99 ans1,ans2;
    if(ask(b[0].ss,b[1].ss,ek)>ask(b[0].ss,b[1].ss,dui))
    {
        ans1=ek;
        if(b[0].ff>b[1].ff)
            ans2=b[0].ss;
