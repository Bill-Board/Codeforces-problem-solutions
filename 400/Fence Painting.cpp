...
void solve()
{
    /// code is here->
    v99 n,m;read(n,m);
    vll a(n),b(n),c(m),ko[n+1],na[n+1];
    read(a);read(b);read(c);
    rep0(i,0,n)
    {
        if(a[i]==b[i])ko[a[i]].pb(i+1);
        else na[b[i]].pb(i+1);
    }
    v99 visi[m+2]={};
    rep0(i,0,m){
        if(!na[c[i]].empty())visi[i+1]=na[c[i]].back(),ko[c[i]].pb(na[c[i]].back()),na[c[i]].pop_back();
        else if(!ko[c[i]].empty())visi[i+1]=ko[c[i]].back();
    }
    for(v99 i=m;i>0;i--)
    {
        if(visi[i]==0)visi[i]=visi[i+1];
        if(visi[i]==0){print("NO");return;}
    }
    rep1(i,1,n)if(!na[i].empty()){print("NO");return;}
    print("YES");
    rep1(i,1,m)cout<<visi[i]<<whp ses[i==m];
}
