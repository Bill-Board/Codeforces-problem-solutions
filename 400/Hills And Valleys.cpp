...
{
    if(i>1 and i+1<=n)
        return (a[i-1]<a[i] and a[i]>a[i+1]) || ( a[i-1]>a[i] and a[i]<a[i+1]);
    else return false;
}
...
void solve()
{
    ...
    v99 koita=0;read(n);
    rep1(i,1,n)cin>>a[i];
    for(v99 i=2;i<n;i++)koita+=alabu(i);
    v99 cr_h=koita;
    for(v99 i=1;i<=n;i++)
    {
        vector<v99>b({a[i]});
        ...
        for(auto j:b)
        {
            ...
            {
                v99 nw=j+k;
                v99 val=koita-alabu(i)-alabu(i-1)-alabu(i+1);
                a[i]=nw;
                cr_h=min(cr_h,val+alabu(i)+alabu(i-1)+alabu(i+1));
                a[i]=old;
            }
        }
    }
