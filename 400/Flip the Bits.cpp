...
void solve()
{
    ...
    rep0(i,1,p.size())
    {
        v99 len=p[i]-p[i-1],s=0;
        for(v99 j=p[i-1]+1;j<=p[i];j++)s+=abs(a[j]-b[j]);
        if(s!=0 and s!=len)return print("NO");
    }
    rep0(i,p.back()+1,n)if(a[i]!=b[i])return print("NO");
    print("YES");
}
