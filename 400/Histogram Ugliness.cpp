...
void solve()
{
    ...
    rep1(i,1,n)read(a[i]),pq.push({a[i],i});
    while(pq.size())
    {
        v99 val=pq.top().ff,ind=pq.top().ss;pq.pop();
        v99 m=max(a[ind+1],a[ind-1]);
        if(m<val)sum+=val-m,a[ind]=m;
    }
    rep1(i,1,n+1)sum+=abs(a[i]-a[i-1]);
    print(sum);
}
