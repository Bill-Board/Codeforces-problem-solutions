...
///-------------------------------------------------------------------------------------------------------
v99 n,a[504],ans[505][505];
bool ok(v99 indx,v99 indy)
{
    return (indx>0 and indx<=n and indy>0 and indy<=n);
}
void dfs(v99 indx,v99 indy,v99 val,v99 koita)
{
    if(koita==0)return;
    if(ok(indx,indy-1) and ans[indx][indy-1]==0)
        ans[indx][indy-1]=val,dfs(indx,indy-1,val,koita-1);
    else if(ok(indx+1,indy) and ans[indx+1][indy]==0)
        ans[indx+1][indy]=val,dfs(indx+1,indy,val,koita-1);
}
...
void solve()
{
    ...
    for(v99 i=1;i<=n;i++)
        for(v99 j=1;j<=i;j++)cout<<ans[i][j]<<whp ses[j==i];
}
