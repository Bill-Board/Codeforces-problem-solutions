...
v99 is_pali(string s,char c)
{
    v99 n=s.size(),kot=0;
    for(v99 i=0,j=n-1;i<=j;)
    {
        if(s[i]!=c and s[j]!=c and s[i]!=s[j])return LLONG_MAX;
        else if(s[i]==s[j]){i++;j--;continue;}
        else if(s[i]!=s[j] and s[j]==c)j--,kot++;
        else if(s[i]!=s[j] and s[i]==c)i++,kot++;
    }
    return kot;
}
...
void solve()
{
    ...
    string s;read(s);
    set<char>c;
    rep0(i,0,n)c.insert(s[i]);
    bool ok=false;
    for(v99 i=0;i<n/2;i++)if(s[i]!=s[n-i-1])ok=true;
    if(ok==false)return print(0);
    for(auto i:c)
    {
        cr_h=min(cr_h,is_pali(s,i));
    }
