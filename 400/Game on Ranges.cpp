...
#define mxi 200003
#define mp make_pair
#define pii pair<pair<v99, v99>,v99 >
#define pf printf
#define sf scanf
#define ff first
...
void solve()
{
    ...
    for(v99 i=0;i<n;i++)
    {
        v99 l=a[i].ff.ff,r=a[i].ff.ss,ind=a[i].ss;
        //print(l,r);
        vll pre(n+2,0);
        for(v99 j=i+1;j<n;j++)pre[a[j].ff.ff]++,pre[a[j].ff.ss+1]--;
        rep1(j,1,n)pre[j]+=pre[j-1];
        //print(pre);
        rep1(j,l,r)
        {
            if(pre[j]==0 and s.find(j)==s.end())
            {
                print(l,r,j);
                s.insert(j);
                break;
