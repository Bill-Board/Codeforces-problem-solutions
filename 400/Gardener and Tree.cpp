...
void solve()
{
    ...
    rep1(i,1,n)if(tom[i].size()==1)a.pb(i);
    while(k--)
    {
        vll b;
        if(a.empty())break;
        for(auto i:a)
        {
            ...
            v99 kot=*tom[i].begin();
            tom[kot].erase(tom[kot].find(i));
            tom[i].erase(tom[i].find(kot));
            if(tom[kot].size()==1)b.pb(kot);
        }
        a.clear();
        a=b;
    }
    v99 cr_h=0;
    for(auto i:a)tom[i].insert(1LL);
    for(v99 i=1;i<=n;i++)
    {
        if(tom[i].size()>0)cr_h++;
    }
    print(cr_h);
}
