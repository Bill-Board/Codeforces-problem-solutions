...
void ppp()
{
    rep1(i,0,n)
    {
        rep1(j,0,m)visi[i][j]=0;
    }
}

v99 kot(v99 x,v99 y)
{
    v99 ans=0, y1=y;//print(x,y);
    for(v99 r=1;;r+=2)
    {
        if(x>n or y<1 or y1>m)break;
        v99 kot=visi[x][y1]-visi[x][y-1];
        //cout<<kot<<whp<<r<<ses;
        if(kot!=r)break;
        ans++;
        ++x;--y;++y1;
...
void solve()
{
    ...
    read(n,m);
    ppp();
    rep1(i,1,n)rep1(j,1,m)cin>>s[i][j];
    v99 cr_h=0;
    rep1(i,1,n)
    {
        rep1(j,1,m)visi[i][j]=visi[i][j-1]+(s[i][j]=='*');
    }
    //rep1(i,1,n){rep1(j,1,m)cout<<visi[i][j]<<whp;cout<<ses;}
    rep1(i,1,n){ rep1(j,1,m)if(s[i][j]=='*')cr_h+=kot(i,j);}
    print(cr_h);
}
